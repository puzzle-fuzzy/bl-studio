...
> - **包管理 / 任务编排：bun + turbo**。一切安装、脚本、测试都走 bun。
...
> - Node 版本固定为 24.x（`.node-version` 为 24.12.0；`import.meta.main` 是原生特性，不要再引入运行时探测 helper）。

## 常用命令

```bash
bun install
bun run dev              # turbo 并行：api(bun,5003) / worker(tsx) / studio(vite,5002)
bun run lint             # Biome lint（TS/TSX/JS，跨平台）
bun run typecheck        # 各包 tsc --noEmit（turbo 并行）
bun run typecheck:root   # 根作用域 tsc --noEmit：scripts/ + 根 tests/（per-package typecheck 不含这两处）
bun run test             # 根契约测试 + 全仓 vitest（串行，共享 test DB）
bun run verify           # check:db-migrations + boundaries + manifests + lint + typecheck + test
bun run check:boundaries # 包边界
bun run check:manifests  # manifest 一致性
...
bun run check:db-migrations # schema↔迁移链对账（drizzle-kit generate 离线比较；见 scripts/verify/check-db-migrations.ts）
...
# 改了 schema.ts 后正式流程是 `bunx drizzle-kit generate --config packages/db/drizzle.config.ts` 提交迁移，
...
**先读 `docs/04-deployment-playbook.md`（部署手册：正确流程 + 踩坑清单）和 `docs/03-ops.md`（运维：日志/备份/回滚/故障排查）。** 一键发布：`bun run verify && bun run deploy:prod`。日志栈 Loki + Alloy + Grafana 挂在 `logs.yxswy.com`（观测栈在 `observability` profile，`bun run prod:observability:up` 启用）。

**生产环境的关键事实（易踩坑，务必先知道）：**
- 服务器是**共享机**（SSH 别名 `yxswy-server`，主机/凭据只存于本机 `~/.ssh/config`，不入库），同时跑着多个其它服务。
- **HTTPS 边缘 = 宿主机 nginx + certbot**（80/443 被独占），**不是 Caddy**。站点配置在 `/etc/nginx/conf.d/*.yxswy.com.conf`，反代到 127.0.0.1:5002（studio web 容器）/5300（grafana）。
...
- 安全红线：`deploy/env/.env.dev`、`deploy/env/.env.test`、`deploy/env/.env.prod` 均 gitignored，**绝不打印/提交任何凭据值**。
...
改跨包 import 后必须 `bun run check:boundaries`。
...
3. `bun run verify` 全绿 → `bun run deploy:prod` 部署。
...
- **状态**：服务端数据使用 TanStack Query，登录态与少量纯客户端状态使用 zustand；登出时统一 `resetPrivateData()`（注册表机制）。
...
- **UI 组件**一律优先使用 `packages/ui` 的共享原语；样式用 Tailwind v4。
...
- 与 studio 同源部署（共享 `packages/ui` / `packages/lib-client` / `api-client`），**同源 `/admin` 挂载**（nginx 无需新域名/证书/跨域）。
...
- 新增跨 app UI 原语时放入 `packages/ui`，避免 studio/admin/writer/canvas 各自复制一份。

## 测试约定

- 后端包测试放 `tests/`（与 `src/` 同级），vitest。曾经是 `bun:test`，已全部迁移。
- 根测试 `tests/` + `scripts/**/*.test.ts` 由 `bun run test:root` 跑。
- 改代码后跑 `bun run typecheck` + 对应包测试 + `bun run check:boundaries`。
