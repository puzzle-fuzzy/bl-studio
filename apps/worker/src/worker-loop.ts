...
export class WorkerLoop {
    ...
    try {
      while (this.running) {
        ...
        try {
          ...
          this.logger.error('worker.iteration_failed', {
            error: errorMessage(error),
          })
  ...
  private startWorkerHeartbeat(): () => Promise<void> {
    ...
    if (register === undefined) {
      this.logger.warn('worker.heartbeat_unavailable', {
    ...
    try {
      const registration = register.call(this.config.repository, {
        workerId: this.config.workerId,
        startedAt,
      })
  ...
  private async sweepStaleGenerations(): Promise<void> {
    ...
    try {
      ...
      this.logger.error('stale_generations.list_failed', {
        error: errorMessage(error),
      })
      return
    }
      ...
      try {
        await repo.failGeneration({
          ...
          error: {
            category: 'system',
            message:
              'Generation was swept because its task reached a terminal failure state but the record was left in flight',
            retriable: false,
            code: 'GENERATION_STALE_SWEPT',
          },
          now: currentIso(),
        })
        this.logger.warn('stale_generations.swept', {
          recordId: record.id,
          userId: record.userId,
        })
      } catch (error) {
        this.logger.error('stale_generations.fail_failed', {
          recordId: record.id,
          error: errorMessage(error),
        })
      }
  ...
  private async sweepStaleReservations(): Promise<void> {
    ...
    try {
      ...
      const result = await ledger.releaseStaleReservations({
        olderThan,
        confirm: true,
      })
      ...
      this.logger.error('stale_reservations.sweep_failed', {
        error: errorMessage(error),
      })
    }
  }
  ...
  switch (outcome.status) {
    ...
    case 'polling':
      return withNextInput(
        transitionTask(task, {
          type: 'succeed',
          output: { nextPollAt: outcome.nextPollAt },
          now,
        }),
      )
    case 'failed':
