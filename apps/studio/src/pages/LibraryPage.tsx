...
export function LibraryPage() {
  ...
  const assetList = useAssetList(query)
  const loadMore = () => {
  ...
  const items = assetList.data?.pages.flatMap(page => page.items) ?? []
  const pendingThumbnails = hasPendingThumbnails(items)
  usePendingThumbnailRefresh(pendingThumbnails, () => void assetList.refetch())
