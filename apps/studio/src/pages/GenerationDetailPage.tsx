...
import { DropdownMenu, DropdownMenuContent, DropdownMenuItem, DropdownMenuLabel, DropdownMenuTrigger } from '@bailian-studio/ui'
...
import { useGenerationDetail, useInvalidateGenerations } from '@/hooks/use-generations'
...
export function GenerationDetailPage() {
  ...
  const refreshRecord = useInvalidateGenerations()
  // P1-04：详情页记录跟随 store —— SSE/降级轮询把最新 record 合并进 store.records 后，
  // 这里取到的对象引用随之更新，页面开着也能看到 queued→succeeded 全过程。
  const storeRecord = useGenerationDetail(id ?? undefined).data ?? null
...
function DetailContent({
  ...
  const handleSavePrompt = async () => {
    ...
    try {
      ...
      const format = referenceFormatOf(selectModelById(catalogModels, record.modelId))
...
function ParamsCard({ record }: { record: GenerationRecord }) {
  ...
  const refAssets = useReferenceAssets(refIds)
