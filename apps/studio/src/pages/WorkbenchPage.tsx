...
export function WorkbenchPage() {
  ...
  const assetList = useCreativeAssetList({})
  ...
  useEffect(() => {
    if (assetList.error !== null) notifyError(assetList.error)
  }, [assetList.error])

  const projects = projectQuery?.items ?? []
  const recentAssets = (assetList.data?.pages.flatMap(page => page.items) ?? []).slice(0, 6)
    ...
    <div className="relative min-h-full overflow-hidden">
      <div className="mx-auto flex w-full max-w-[1660px] flex-col gap-5">
        ...
        <section className="space-y-4">
          ...
          ) : assetList.error !== null && recentAssets.length === 0 ? (
            <Card className="border-dashed bg-card/60">
