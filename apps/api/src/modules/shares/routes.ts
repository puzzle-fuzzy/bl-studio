...
export function createShareRoutes(deps: ApiDependencies) {
	...
	return new Elysia({ prefix: "/api/shares" })
		.get("/generations/:shareId", async ({ request, params, set }) => {
			...
			const shared =
				await deps.publicShareRepository.getPublicSharedGeneration(shareId);
			if (shared === undefined) {
				set.status = 404;
				return requestErrorResponseBody(
					request,
			...
			const repository = deps.publicShareRepository;
			const storage = deps.storage;
			const artifacts = await Promise.all(
				shared.artifacts.map(
					async (artifact): Promise<PublicArtifactWithReadUrl> => {
						if (artifact.status !== "stored") return artifact;
						const stored = await repository.getPublicSharedArtifact(
							shareId,
							artifact.id,
						);
		...
		.get(
			"/generations/:shareId/artifacts/:artifactId",
			async ({ request, params, set }) => {
				...
				if (
					...
					return requestErrorResponseBody(
						request,
						"INVALID_SHARE_ARTIFACT",
						"Share and artifact ids are required",
						set,
					);
				}

				const artifact =
					await deps.publicShareRepository.getPublicSharedArtifact(
						shareId,
						artifactId,
					);
				...
				try {
					...
					return requestErrorResponseBody(
						...
						error instanceof LocalFileTooLargeError
							...
							: code === "ENOENT"
								? "Shared artifact not found"
								: "Failed to read shared artifact",
						set,
						error instanceof LocalFileTooLargeError
