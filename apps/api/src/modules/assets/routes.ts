...
export function createAssetRoutes(deps: ApiDependencies) {
  ...
  .post('/upload', async ({ request }) => {
    ...
    try {
      ...
      const asset = await uploadAsset({
        ...
        repository: deps.generationRepository,
  ...
  .post('/import', async ({ request, body }) => {
    ...
    try {
      ...
      const asset = await importAsset({ input, userId: user.id, repository: deps.generationRepository })
      await recordApiAuditEvent(deps.generationRepository, request, {
        userId: user.id,
        action: 'asset.import',
        outcome: 'succeeded',
    ...
    return {
      ...
      data: {
        ...
        ...(deps.assetConfig.maxMediaDurationSeconds !== undefined
          ? { maxMediaDurationSeconds: deps.assetConfig.maxMediaDurationSeconds }
          : {}),
  ...
  .get('/', async ({ request, query }) => {
    ...
    const modelIds = normalizedQuery === undefined || normalizedQuery.length === 0
      ...
      : listModels()
          ...
          .map(model => model.id)
    const data = await deps.generationRepository.listUnifiedAssets(user.id, {
      sort,
  ...
  .get('/:id', async ({ request, params, set }) => {
    ...
    if (asset === undefined) {
      set.status = 404
      return requestErrorResponseBody(request, 'ASSET_NOT_FOUND', 'Asset not found', set)
    }
    return {
      success: true,
  ...
  .delete('/:id', async ({ request, params, set }) => {
    const user = await requireAuthUser(request, deps.authService)
    const deleted = await deps.generationRepository.softDeleteUserAsset({
      userId: user.id,
      assetId: params.id,
    })
    if (!deleted) {
...
async function assetWithDownloadUrl(
  ...
  return {
    ...
    downloadUrl: await storage.createReadUrl({
      ...
      downloadFileName: assetDownloadFileName(item.fileName, item.id, item.mimeType),
    }),
  }
}
...
async function assetWithReadUrl(
  ...
  const publicItem = {
    ...
    ...(item.declaredResolution !== undefined ? { declaredResolution: item.declaredResolution } : {}),
    ...(item.fileName !== undefined ? { fileName: item.fileName } : {}),
    ...(item.recordId !== undefined ? { recordId: item.recordId } : {}),
    ...(item.modelId !== undefined ? { modelId: item.modelId } : {}),
    ...(item.thumbnailStatus !== undefined ? { thumbnailStatus: item.thumbnailStatus } : {}),
  ...
  const withOriginal = item.storageKey === undefined
    ...
    : {
        ...publicItem,
        url: await storage.createReadUrl({ key: item.storageKey, expiresInSeconds: 3600 }),
      }
