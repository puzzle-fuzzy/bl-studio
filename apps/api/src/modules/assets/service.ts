...
export async function uploadAsset(args: {
	...
	if (file.size > config.maxAssetSizeBytes) {
		throw new ValidationError(
			`File exceeds maximum size of ${config.maxAssetSizeBytes} bytes`,
			"file",
		);
	}
	...
	const kind: AssetKind = isAssetKind(kindParamText)
		? kindParamText
		: mimeToKind(mimeType);
	...
	const stored =
		storage.writeObjectMultipart !== undefined
	...
	const input: CreateUserAssetInput = {
		...
		enqueueThumbnail:
			stored.provider === "local" && (kind === "image" || kind === "video"),
...
export async function importAsset(args: {
	...
	userId: string;
	repository: Pick<AssetRepository, "createUserAsset">;
