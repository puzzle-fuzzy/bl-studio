...
const fakeGenerationRepository = {} as GenerationRepository;
...
describe("notifications routes", () => {
	...
	it("未读数只统计本人未读", async () => {
		const response = await app.handle(
			authed("/api/notifications/unread-count"),
		);
	...
	it("标记已读；越权标记他人通知 → 404", async () => {
		const ok = await app.handle(
			authed("/api/notifications/n-1/read", { method: "POST" }),
		);
		expect(ok.status).toBe(200);
		const other = await app.handle(
			authed("/api/notifications/n-3/read", { method: "POST" }),
		);
	...
	it("全部已读", async () => {
		const response = await app.handle(
			authed("/api/notifications/read-all", { method: "POST" }),
		);
