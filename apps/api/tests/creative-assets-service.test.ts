...
import { createCreativeAssetApplicationService } from '../src/modules/creative-assets/service'
...
describe('creative asset use cases', () => {
  it('turns an owned-resource miss into the stable project not-found error', async () => {
    ...
    const useCases = createCreativeAssetApplicationService({ repository: partialRepository({ getProject }) })
  ...
  it('keeps the authenticated user and state operation inside the application boundary', async () => {
    ...
    const useCases = createCreativeAssetApplicationService({ repository: partialRepository({ transitionVersion, removeReference }) })
  ...
  it('keeps repository errors intact for state and validation failures', async () => {
    ...
    const useCases = createCreativeAssetApplicationService({ repository: partialRepository({ transitionVersion }) })

    await expect(useCases.transitionVersion({ userId: 'user-a', assetVersionId: 'version-1', status: 'approved' }))
      .rejects.toBe(failure)
  })

  it('uses the explicit publish boundary for approval transitions', async () => {
    const transitionVersion = vi.fn<CreativeAssetRepository['transitionVersion']>().mockResolvedValue({} as never)
    const service = createCreativeAssetApplicationService({ repository: partialRepository({ transitionVersion }) })

    await service.publishVersion({ userId: 'user-a', assetVersionId: 'version-1' })
