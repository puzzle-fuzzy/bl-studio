...
const INITIAL_MESSAGES: WriterMessage[] = [
	...
	},
	{
		...
		role: "user",
		content:
			"我想写一个关于“下班后重新开始”的短篇故事，希望它温暖一点，但不要太鸡汤。",
		time: "刚刚",
	},
	{
		id: "mock-assistant",
...
export function WritingPage() {
	...
	return (
		<div className="mx-auto flex min-h-[calc(100svh-2rem)] w-full max-w-[1500px] flex-col gap-5">
			...
			<div className="grid min-h-0 flex-1 gap-4 lg:grid-cols-[minmax(0,1fr)_320px]">
				<Card className="flex min-h-[620px] min-w-0 flex-col gap-0 overflow-hidden lg:min-h-0">
					<CardHeader className="flex shrink-0 flex-row items-center justify-between border-b py-4">
					...
					<div className="flex min-h-0 flex-1 flex-col">
						...
						<form
							...
							className="shrink-0 bg-muted/20 p-4 md:p-5"
						>
							<div className="mx-auto flex max-w-3xl flex-col gap-3 rounded-2xl border bg-background p-3 shadow-sm transition-shadow focus-within:ring-2 focus-within:ring-ring/20">
								<Textarea
									value={draft}
				...
				<Card className="flex min-h-[360px] flex-col gap-0 overflow-hidden lg:min-h-0">
					<CardHeader className="flex shrink-0 flex-row items-start justify-between border-b py-4">
						<div className="flex items-start gap-3">
							<div className="flex size-8 shrink-0 items-center justify-center rounded-lg bg-muted text-muted-foreground">
								<ImageIcon className="size-4" />
							</div>
							<div>
								<CardTitle className="text-sm">图片资产</CardTitle>
					...
					<CardContent className="flex min-h-0 flex-1 flex-col p-4">
						<div className="flex min-h-64 flex-1 flex-col items-center justify-center rounded-xl border border-dashed bg-muted/20 px-6 text-center">
							<div className="mb-4 flex size-12 items-center justify-center rounded-2xl bg-background text-muted-foreground shadow-sm ring-1 ring-border">
								<ImageIcon className="size-6" />
							</div>
							<h2 className="text-sm font-medium">这里还没有图片资产</h2>
