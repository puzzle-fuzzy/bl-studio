...
export function CreativeAssetPickerDialog({
  ...
  useEffect(() => {
    ...
    apiClient.listCreativeAssets({ limit: 100, ...(type === 'all' ? {} : { type }), ...(query.trim() ? { q: query.trim() } : {}) })
      ...
      .catch(loadError => { if (!cancelled) setError(userErrorMessage(loadError)) })
