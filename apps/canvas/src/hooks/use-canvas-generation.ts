...
export function useCanvasGeneration(
  nodeId: string,
  onStatusChange: (status: 'generating' | 'ready' | 'error', result?: MediaResult, error?: string) => void,
) {
  ...
  }, [clearTimer, nodeId, onStatusChange, stopPolling])

  return { generate, stopPolling }
}
