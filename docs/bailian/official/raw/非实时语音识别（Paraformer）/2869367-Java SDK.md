...
[核心类（Transcription）](/zh/model-studio/paraformer-recorded-speech-recognition-java-sdk#adcb5e9bddbyq)提供了异步提交任务、同步等待任务结束和异步查询任务执行结果的接口。可通过如下两种调用方式进行非实时语音识别：
...
1.  配置[请求参数](/zh/model-studio/paraformer-recorded-speech-recognition-java-sdk#48ea212b1d08r)。
    
2.  实例化[核心类（Transcription）](/zh/model-studio/paraformer-recorded-speech-recognition-java-sdk#adcb5e9bddbyq)。
    
3.  调用[核心类（Transcription）](/zh/model-studio/paraformer-recorded-speech-recognition-java-sdk#adcb5e9bddbyq)的`asyncCall`方法异步提交任务。
...
4.  调用[核心类（Transcription）](/zh/model-studio/paraformer-recorded-speech-recognition-java-sdk#adcb5e9bddbyq)的`wait`方法同步等待任务结束。
    ...
    `wait`返回[任务执行结果（TranscriptionResult）](/zh/model-studio/paraformer-recorded-speech-recognition-java-sdk#11a082e1d9ijq)。
...
4.  循环调用[核心类（Transcription）](/zh/model-studio/paraformer-recorded-speech-recognition-java-sdk#adcb5e9bddbyq)的`fetch`方法直到获取最终的任务结果。
    
    当任务状态为`SUCCEEDED`或`FAILED`时，停止轮询并处理结果。
    
    `fetch`返回[任务执行结果（TranscriptionResult）](/zh/model-studio/paraformer-recorded-speech-recognition-java-sdk#11a082e1d9ijq)。
...
注：`phraseId`为v1版本模型热词方案，不支持v2及后续系列模型。支持该方式热词的模型列表请参考[Paraformer语音识别热词定制与管理](/zh/model-studio/paraformer-asr-phrase-manager)。
...
有关`speaker_id`的示例，请参见[识别结果说明](/zh/model-studio/paraformer-recorded-speech-recognition-java-sdk#a9021178ccl7s)。
...
[子任务执行结果（TranscriptionTaskResult）](/zh/model-studio/paraformer-recorded-speech-recognition-java-sdk#86b568aa3asuj)

 | 

获取[子任务执行结果（TranscriptionTaskResult）](/zh/model-studio/paraformer-recorded-speech-recognition-java-sdk#86b568aa3asuj)。
...
“`code`”为错误码，“`message`”为错误信息，只有异常情况才有这两个字段，您可以通过这两个字段，对照[错误码](/zh/model-studio/paraformer-recorded-speech-recognition-java-sdk#a370386972oa4)排查问题。
...
JSON数据中各字段含义请参见[识别结果说明](/zh/model-studio/paraformer-recorded-speech-recognition-java-sdk#a9021178ccl7s)。
...
[任务执行结果（TranscriptionResult）](/zh/model-studio/paraformer-recorded-speech-recognition-java-sdk#11a082e1d9ijq)
...
如遇代码报错问题，请根据[错误码](/zh/model-studio/paraformer-recorded-speech-recognition-java-sdk#a370386972oa4)中的信息进行排查。

#### Q：识别结果和语音播放不同步怎么办？

将[请求参数](/zh/model-studio/paraformer-recorded-speech-recognition-java-sdk#48ea212b1d08r)`timestampAlignmentEnabled`设为`true`将启用时间戳校准功能，能够让识别结果和语音播放同步。
