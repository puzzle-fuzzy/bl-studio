...
**用户指南：**关于模型介绍和选型建议请参见[实时语音合成-CosyVoice /Sambert](/zh/model-studio/text-to-speech)。
...
[SpeechSynthesizer类](/zh/model-studio/sambert-python-sdk#adcb5e9bddbyq)提供了非流式调用和单向流式调用的接口。请根据业务场景选择合适的调用方式：
...
直接调用[SpeechSynthesizer类](/zh/model-studio/sambert-python-sdk#adcb5e9bddbyq)的`call`方法进行语音合成。`call`方法可对[请求参数](/zh/model-studio/sambert-python-sdk#fdafc9b5535f3)进行设置，注意此时不要设置`callback`参数。

任务完成后该方法返回[音频数据和时间戳信息（SpeechSynthesisResult）](/zh/model-studio/sambert-python-sdk#a727da82951p6)。
...
1.  实例化[回调接口（ResultCallback）](/zh/model-studio/sambert-python-sdk#3639e1cb40mxi)。
2.  调用[SpeechSynthesizer类](/zh/model-studio/sambert-python-sdk#adcb5e9bddbyq)的`call`方法进行语音合成。`call`方法可对[请求参数](/zh/model-studio/sambert-python-sdk#fdafc9b5535f3)进行设置，注意此时要设置`callback`参数。
...
请求参数通过[SpeechSynthesizer类](/zh/model-studio/sambert-python-sdk#adcb5e9bddbyq)的`call`方法进行设置。
...
指定用于语音合成的音色模型名，完整列表参见[模型列表](/zh/model-studio/sambert-python-sdk#a737f8b6f8gx0)。
...
指定合成音频的采样率（单位：Hz），建议使用模型默认采样率（参见[模型列表](/zh/model-studio/sambert-python-sdk#a737f8b6f8gx0)），如果不匹配，服务会进行必要的升降采样处理。
...
callback的实现请参见[回调接口（ResultCallback）](/zh/model-studio/sambert-python-sdk#3639e1cb40mxi)
...
-   `model`：模型名，参见[模型列表](/zh/model-studio/sambert-python-sdk#a737f8b6f8gx0)
-   `text`：待合成文本
-   `callback`：[回调接口（ResultCallback）](/zh/model-studio/sambert-python-sdk#3639e1cb40mxi)
-   `workspace`：DashScope workspace id，不必关注
-   `kwargs`：[请求参数](/zh/model-studio/sambert-python-sdk#fdafc9b5535f3)，如`format`等
...
`call`方法能够对[请求参数](/zh/model-studio/sambert-python-sdk#fdafc9b5535f3)进行设置。

 |

### 回调接口（`ResultCallback`）

[单向流式调用](/zh/model-studio/sambert-python-sdk#ba023aacfbr84)时，服务端会通过回调的方式，将关键流程信息和数据返回给客户端。您需要实现回调方法，处理服务端返回的信息或者数据。
...
`result`：[音频数据和时间戳信息（SpeechSynthesisResult）](/zh/model-studio/sambert-python-sdk#a727da82951p6)
...
当前合成的句子对应的[时间戳信息](/zh/model-studio/sambert-python-sdk#6a6d7363490w6)
...
所有句子对应的[时间戳信息](/zh/model-studio/sambert-python-sdk#6a6d7363490w6)
...
包含的字级别时间戳信息，需要请求中`word_timestamp_enabled`也设置为true*。*
...
包含的音素级别时间戳信息，需要请求中`phoneme_timestamp_enabled`也设置为true*。*
...
在使用API过程中，如果调用失败并返回错误信息，请参见[错误码](/zh/model-studio/error-code)进行解决。
