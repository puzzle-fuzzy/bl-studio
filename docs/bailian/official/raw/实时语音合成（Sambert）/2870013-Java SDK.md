...
`wss://{WorkspaceId}.cn-beijing.maas.aliyuncs.com/api-ws/v1/inference`，调用时请将`{WorkspaceId}`替换为真实的[Workspace ID](/zh/model-studio/obtain-the-app-id-and-workspace-id#732535cfc959h)。
...
[SpeechSynthesizer类](/zh/model-studio/sambert-java-sdk#adcb5e9bddbyq)提供了非流式调用和单向流式调用的接口。请根据业务场景选择合适的调用方式：
...
实例化[SpeechSynthesizer类](/zh/model-studio/sambert-java-sdk#adcb5e9bddbyq)，调用`call`方法绑定[请求参数](/zh/model-studio/sambert-java-sdk#a96bfa6340jdd)，进行合成并获取二进制音频数据。
...
实例化[SpeechSynthesizer类](/zh/model-studio/sambert-java-sdk#adcb5e9bddbyq)，调用`call`方法绑定[请求参数](/zh/model-studio/sambert-java-sdk#a96bfa6340jdd)和[回调接口（ResultCallback）](/zh/model-studio/sambert-java-sdk#3639e1cb40mxi)并开始语音合成，通过[回调接口（ResultCallback）](/zh/model-studio/sambert-java-sdk#3639e1cb40mxi)的`onEvent`方法实时获取合成结果。

语音合成完成后（[回调接口（ResultCallback）](/zh/model-studio/sambert-java-sdk#3639e1cb40mxi)的`onComplete`方法被回调之后），还可以调用[SpeechSynthesizer类](/zh/model-studio/sambert-java-sdk#adcb5e9bddbyq)的`getAudioData`和`getTimestamps`方法，一次性获取完整的音频和时间戳结果。
...
以下示例展示了通过`Flowable`对象的`blockingForEach`接口，阻塞式地获取每次流式返回的[音频数据和时间戳信息（SpeechSynthesisResult）](/zh/model-studio/sambert-java-sdk#a727da82951p6)。

您也可以在Flowable的所有流式数据返回完成后，通过[SpeechSynthesizer类](/zh/model-studio/sambert-java-sdk#adcb5e9bddbyq)的`getAudioData`和`getTimestamps`方法分别获取完整的合成结果和完整的时间戳。
...
在DashScope Java SDK中，采用了OkHttp3的连接池技术，以减少重复建立连接的开销。详情请参见[高并发场景](/zh/model-studio/sambert-in-high-concurrency-scenarios)。

## 请求参数

通过`SpeechSynthesisParam`的链式方法配置模型、待合成文本等参数。配置完成的对象传入[SpeechSynthesizer类](/zh/model-studio/sambert-java-sdk#adcb5e9bddbyq)的`call`方法中使用。
...
指定用于语音合成的音色模型名，完整列表请参见[模型列表](/zh/model-studio/sambert-java-sdk#57d33631f7doi)。
...
指定合成音频的采样率（单位：Hz），建议使用模型默认采样率（参见[模型列表](/zh/model-studio/sambert-java-sdk#57d33631f7doi)），如果不匹配，服务会进行必要的升降采样处理。
...
`param`：[请求参数](/zh/model-studio/sambert-java-sdk#a96bfa6340jdd)
...
-   `param`：[请求参数](/zh/model-studio/sambert-java-sdk#a96bfa6340jdd)
-   `callback`：[回调接口（ResultCallback）](/zh/model-studio/sambert-java-sdk#3639e1cb40mxi)
...
[句子级别时间戳信息（Sentence）](/zh/model-studio/sambert-java-sdk#6a6d7363490w6)的`List`集合

 | 

获取完整的[句子级别时间戳信息（Sentence）](/zh/model-studio/sambert-java-sdk#6a6d7363490w6)。
...
[单向流式调用](/zh/model-studio/sambert-java-sdk#ba023aacfbr84)时，通过回调接口`ResultCallback`获取合成结果。
...
`result`：[音频数据和时间戳信息（SpeechSynthesisResult）](/zh/model-studio/sambert-java-sdk#a727da82951p6)
...
[非流式调用](/zh/model-studio/sambert-java-sdk#8341058094tc3)：响应结果为二进制音频数据。

[单向流式调用](/zh/model-studio/sambert-java-sdk#ba023aacfbr84)：响应结果为[音频数据和时间戳信息（SpeechSynthesisResult）](/zh/model-studio/sambert-java-sdk#a727da82951p6)。
...
[字级别时间戳信息（Word）](/zh/model-studio/sambert-java-sdk#216af8dfafag2)的`List`集合
...
[音素级别时间戳信息（Phoneme）](/zh/model-studio/sambert-java-sdk#05e7893c08and)的`List`集合
