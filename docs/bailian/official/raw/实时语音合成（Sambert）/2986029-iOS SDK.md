...
-   **回调驱动：**通过 [`NeoNuiTtsDelegate`](/zh/model-studio/sambert-ios-sdk#8b030fec74e01) 协议接收事件和数据
    
-   **事件类型：**
    -   [`TTS_EVENT_START`](/zh/model-studio/sambert-ios-sdk#981ff433acpmr)：合成任务开始
    -   [`onNuiTtsUserdataCallback`](/zh/model-studio/sambert-ios-sdk#bc71fe2545pfy)：音频数据返回
    -   [`TTS_EVENT_END`](/zh/model-studio/sambert-ios-sdk#981ff433acpmr)：合成任务结束
    -   [`TTS_EVENT_ERROR`](/zh/model-studio/sambert-ios-sdk#981ff433acpmr)：合成出错

### 使用流程

1.  [`nui_tts_initialize()`](/zh/model-studio/sambert-ios-sdk) - 初始化SDK，设置回调接口和连接参数
2.  [`nui_tts_set_param()`](/zh/model-studio/sambert-ios-sdk) - 设置语音合成效果参数（模型、音色、音量等）
3.  [`nui_tts_play()`](/zh/model-studio/sambert-ios-sdk) - 启动语音合成任务
4.  [`onNuiTtsUserdataCallback()`](/zh/model-studio/sambert-ios-sdk) - 接收音频数据
5.  [`nui_tts_release()`](/zh/model-studio/sambert-ios-sdk) - 释放SDK资源

### Sambert 方法

#### nui\_tts\_initialize

初始化语音合成SDK实例。SDK为单例模式，在调用 [`nui_tts_release`](/zh/model-studio/sambert-ios-sdk)前禁止重复初始化。
...
[`NuiSdkLogLevel`](/zh/model-studio/sambert-ios-sdk#h2nuisdkloglevel)
...
此参数仅在调用[nui\_tts\_initialize](/zh/model-studio/sambert-ios-sdk)接口时将[`save_log`](/zh/model-studio/sambert-ios-sdk)设为`YES`时生效。此时必须设置日志文件路径，否则将报错。
...
此参数仅在调用[nui\_tts\_initialize](/zh/model-studio/sambert-ios-sdk)接口时将[`save_log`](/zh/model-studio/sambert-ios-sdk)设为`YES`时生效。
...
控制通过日志回调（[onNuiTtsLogTrackCallback](/zh/model-studio/sambert-ios-sdk)）对外发送的日志内容的过滤级别。
...
注意：`log_track_level`与[`level`](/zh/model-studio/sambert-ios-sdk)（通过[nui\_tts\_initialize](/zh/model-studio/sambert-ios-sdk)接口设置）共同决定最终回调的日志。一条日志的级别数值必须同时大于或等于`log_track_level`和[`level`](/zh/model-studio/sambert-ios-sdk)的值，才会被回调。例如，`log_track_level`设为2 (INFO)，[`level`](/zh/model-studio/sambert-ios-sdk)设为3 (WARNING)，则只有WARNING及以上级别（数值>=3）的日志才会被回调。

 |

#### nui\_tts\_set\_param

以键值对的形式设置参数。在 [`nui_tts_play`](/zh/model-studio/sambert-ios-sdk)之前调用。
...
暂停当前语音合成任务。任务暂停后，可通过 [`nui_tts_resume`](/zh/model-studio/sambert-ios-sdk)恢复，或通过 [`nui_tts_cancel`](/zh/model-studio/sambert-ios-sdk)彻底取消。在任务暂停期间，SDK不支持启动新的合成任务。
...
释放SDK所有内部资源，并强制终止所有正在进行的合成任务。此方法调用后，SDK实例将变为不可用状态，如需再次使用，必须重新调用 [`nui_tts_initialize`](/zh/model-studio/sambert-ios-sdk)进行初始化。
...
[`NuiSdkTtsEvent`](/zh/model-studio/sambert-ios-sdk#981ff433acpmr)
...
错误码，仅在事件 [TTS\_EVENT\_ERROR](/zh/model-studio/sambert-ios-sdk#981ff433acpmr) 中有效。参见[错误码](https://help.aliyun.com/zh/isi/support/error-codes)。
...
1.  初始化 SDK。
2.  按业务需求设置参数：通过[`nui_tts_initialize`](/zh/model-studio/sambert-ios-sdk)接口的`parameters`参数设置连接参数；通过[`nui_tts_set_param`](/zh/model-studio/sambert-ios-sdk)接口设置语音合成效果参数。
3.  调用 `nui_tts_play` 开始语音合成。
4.  在[onNuiTtsUserdataCallback](/zh/model-studio/sambert-ios-sdk)回调中获取音频数据，建议使用流式播放。如需保存本地，按追加模式将音频写入同一文件，直到合成完成。
