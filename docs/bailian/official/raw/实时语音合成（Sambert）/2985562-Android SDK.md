...
**用户指南：**关于模型介绍和选型建议请参见[语音合成-Sambert](/zh/model-studio/text-to-speech)。
...
-   **回调驱动：**通过 [`INativeTtsCallback`](/zh/model-studio/sambert-android-sdk#8b030fec74e01) 接口接收事件和数据
    
-   **事件类型：**
    -   [`TTS_EVENT_START`](/zh/model-studio/sambert-android-sdk#981ff433acpmr)：合成任务开始
    -   [`onTtsDataCallback`](/zh/model-studio/sambert-android-sdk#bc71fe2545pfy)：音频数据返回
    -   [`TTS_EVENT_END`](/zh/model-studio/sambert-android-sdk#981ff433acpmr)：合成任务结束
    -   [`TTS_EVENT_ERROR`](/zh/model-studio/sambert-android-sdk#981ff433acpmr)：合成出错

### 使用流程

1.  [`tts_initialize()`](/zh/model-studio/sambert-android-sdk#ae6d7dd9cfad3) - 初始化SDK，设置回调接口和连接参数
2.  [`setparamTts()`](/zh/model-studio/sambert-android-sdk#a23e0d85d7ymt) - 设置语音合成效果参数（模型、音色、音量等）
3.  [`startTts()`](/zh/model-studio/sambert-android-sdk#7d33691bdb32v) - 启动语音合成任务
4.  [`onTtsDataCallback()`](/zh/model-studio/sambert-android-sdk#bc71fe2545pfy) - 接收音频数据
5.  [`tts_release()`](/zh/model-studio/sambert-android-sdk#44bf12ed9a4g9) - 释放SDK资源

### Sambert 方法

#### tts\_initialize

初始化语音合成SDK实例。SDK为单例模式，在调用 [`tts_release()`](/zh/model-studio/sambert-android-sdk#44bf12ed9a4g9) 前禁止重复初始化。
...
[`INativeTtsCallback`](/zh/model-studio/sambert-android-sdk#8b030fec74e01)
...
控制通过日志回调（[`onTtsLogTrackCallback`](/zh/model-studio/sambert-android-sdk#9c10968457gc6)）对外发送的日志内容的过滤级别。
...
以键值对的形式设置语音合成效果参数。在 [`startTts()`](/zh/model-studio/sambert-android-sdk#7d33691bdb32v) 之前调用。
...
-   1：开启。当 format 为 mp3 时，设为 "1" 可开启SDK内置解码器，此时 [onTtsDataCallback](/zh/model-studio/sambert-android-sdk#bc71fe2545pfy) 将返回解码后的PCM数据。
...
暂停当前语音合成任务。任务暂停后，可通过 [`resumeTts()`](/zh/model-studio/sambert-android-sdk#01f640f02bka9) 恢复，或通过 [`cancelTts()`](/zh/model-studio/sambert-android-sdk#8c464959f9lm9) 彻底取消。在任务暂停期间，SDK不支持启动新的合成任务。
...
释放SDK所有内部资源，并强制终止所有正在进行的合成任务。此方法调用后，SDK实例将变为不可用状态，如需再次使用，必须重新调用 [`tts_initialize()`](/zh/model-studio/sambert-android-sdk#ae6d7dd9cfad3) 进行初始化。
...
[`TtsEvent`](/zh/model-studio/sambert-android-sdk#981ff433acpmr)
...
[错误码](https://help.aliyun.com/zh/isi/support/error-codes)，仅在事件 [TTS\_EVENT\_ERROR](/zh/model-studio/sambert-android-sdk#981ff433acpmr) 中有效。
...
1.  初始化 SDK。
2.  按业务需求设置参数：通过[tts\_initialize](/zh/model-studio/sambert-android-sdk#ae6d7dd9cfad3)接口的`ticket`参数设置连接与控制参数；通过[setparamTts](/zh/model-studio/sambert-android-sdk#a23e0d85d7ymt)接口设置语音合成效果参数。
3.  调用 `startTts` 开始语音合成。
4.  在[onTtsDataCallback](/zh/model-studio/sambert-android-sdk#bc71fe2545pfy)回调中获取音频数据，建议使用流式播放。如需保存本地，按追加模式将音频写入同一文件，直到合成完成。
