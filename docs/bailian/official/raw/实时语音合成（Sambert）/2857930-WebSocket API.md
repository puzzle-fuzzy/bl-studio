...
**重要**URL 必须使用 `wss://` 协议，且固定不变。Authorization 在请求头中设置（参见[请求头](/zh/model-studio/sambert-websocket-api#sb_ws_h2_headers)）。
...
客户端事件和服务端事件的详细说明，请参见[客户端事件](/zh/model-studio/sambert-client-events)和[服务端事件](/zh/model-studio/sambert-server-events)。
...
为提高资源利用率，建议复用 WebSocket 连接处理多个任务，而非为每个任务建立新连接。详细操作请参见[连接复用（WebSocket）](/zh/model-studio/realtime-tts-user-guide#ug_ws_reuse_h3)。
