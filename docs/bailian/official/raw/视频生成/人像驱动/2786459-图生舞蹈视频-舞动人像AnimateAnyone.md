...
| --- | --- | --- | --- | --- | --- |
| 模型调用 | animate-anyone-detect-gen2 | 模型调用，后付费：0.004元/张 | 200张 | 5 | 同步接口无限制 |
| 模型调用 | animate-anyone-template-gen2 | 模型调用，后付费：0.08元/秒 | 1800秒 | 5 | 1（在同一时刻，只有1个作业实际处于运行状态，其他队列中的作业处于排队状态） |
| 模型调用 | animate-anyone-gen2 | 模型调用，后付费：0.08元/秒 | 1800秒 | 5 | 1（在同一时刻，只有1个作业实际处于运行状态，其他队列中的作业处于排队状态） |
| 模型部署 | animate-anyone-detect | 模型独立部署，预付费：10000元/算力单元/月20元/算力单元/小时需部署成功后调用，仅收取部署费用。 | 无 | 5 | 1算力单元支持2并发 |
| 模型部署 | animate-anyone | 模型独立部署，预付费：10000元/算力单元/月20元/算力单元/小时需部署成功后调用，仅收取部署费用。 | 无 | 5 | 1算力单元支持1并发 |
