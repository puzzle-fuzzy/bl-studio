...
**用户指南**：关于模型介绍和选型建议请参见[实时语音合成](/zh/model-studio/realtime-tts-user-guide)
...
**重要**URL 必须使用 `wss://` 协议。Authorization 在请求头中设置（参见[请求头](/zh/model-studio/interactive-process-of-qwen-tts-realtime-synthesis#qwen_ws_header_h2)），模型通过 URL 查询参数 `model` 指定。
...
客户端事件和服务端事件的详细说明，请参见[客户端事件](/zh/model-studio/qwen-tts-realtime-client-events)和[服务端事件](/zh/model-studio/qwen-tts-realtime-server-events)。
