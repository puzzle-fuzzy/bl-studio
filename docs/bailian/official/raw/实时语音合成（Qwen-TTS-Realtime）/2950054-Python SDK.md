本文介绍 DashScope Python SDK 调用[实时语音合成](/zh/model-studio/realtime-tts-user-guide)时的关键接口与请求参数。

**用户指南**：关于模型介绍和选型建议请参见[实时语音合成](/zh/model-studio/realtime-tts-user-guide)或[非实时语音合成](/zh/model-studio/non-realtime-tts-user-guide)。
...
模型名称。参见[支持的模型](/zh/model-studio/realtime-tts-user-guide#4a43cc1bb7kxg)。
...
语音合成所使用的音色。参见[支持的音色](/zh/model-studio/realtime-tts-user-guide#bac280ddf5a1u)。

支持系统音色和专属音色：

-   **系统音色**：仅限千问3-TTS-Instruct-Flash-Realtime、千问3-TTS-Flash-Realtime和千问-TTS-Realtime系列模型。音色效果请参见：[支持的音色](/zh/model-studio/realtime-tts-user-guide#bac280ddf5a1u)。
    
-   **专属音色**
    
    -   [声音复刻（Qwen）](/zh/model-studio/qwen-tts-voice-cloning)功能定制的音色：仅限千问3-TTS-VC-Realtime系列模型
        
    -   [声音设计（Qwen）](/zh/model-studio/qwen-tts-voice-design)功能定制的音色：仅限千问3-TTS-VD-Realtime系列模型
...
千问-TTS-Realtime（参见[支持的模型](/zh/model-studio/realtime-tts-user-guide)）仅支持`pcm`。
...
千问-TTS-Realtime（参见[支持的模型](/zh/model-studio/realtime-tts-user-guide)）仅支持24000。
...
千问-TTS-Realtime（参见[支持的模型](/zh/model-studio/realtime-tts-user-guide)）不支持该参数。
...
设置指令，参见[指令控制](/zh/model-studio/realtime-tts-user-guide#12884a10929p9)。
...
[session.created](/zh/model-studio/qwen-tts-realtime-server-events#05bb97c283l2n)

> 会话已创建

[session.updated](/zh/model-studio/qwen-tts-realtime-server-events#58f078ab61di5)
...
[input\_text\_buffer.cleared](/zh/model-studio/qwen-tts-realtime-server-events#7a04edb8840o7)
...
[input\_text\_buffer.committed](/zh/model-studio/qwen-tts-realtime-server-events#76abc25665qt8)

> 提交文本并触发语音合成

[response.output\_item.added](/zh/model-studio/qwen-tts-realtime-server-events#efd58c096b3bv)

> 响应时有新的输出内容

[response.content\_part.added](/zh/model-studio/qwen-tts-realtime-server-events#141bec9bb11yi)

> 新的输出内容添加到assistant message 项

[response.audio.delta](/zh/model-studio/qwen-tts-realtime-server-events#a2b95643bdvnl)

> 模型增量生成的音频

[response.audio.done](/zh/model-studio/qwen-tts-realtime-server-events#faa3f9bc106tj)

> 完成音频生成

[response.content\_part.done](/zh/model-studio/qwen-tts-realtime-server-events#5244e1dbc9vw6)

> Assistant message 的音频内容流式输出完成

[response.output\_item.done](/zh/model-studio/qwen-tts-realtime-server-events#a922404106vzd)

> Assistant message 的整个输出项流式传输完成

[response.done](/zh/model-studio/qwen-tts-realtime-server-events#91f767e7440pc)
...
[session.finished](/zh/model-studio/qwen-tts-realtime-server-events#ac332f7d78oxg)
...
包括对接口调用的回复响应和模型生成的文本和音频。具体可以参考：[服务端事件](/zh/model-studio/qwen-tts-realtime-server-events)
