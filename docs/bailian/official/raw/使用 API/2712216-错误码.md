...
详情请参见[模型监控](/zh/model-studio/model-telemetry)文档。
...
**解决方案：**请将`enable_thinking`参数设置为`false`，或者改用[流式输出](/zh/model-studio/stream)方式调用思考模式模型。
...
**原因：** 模型仅支持[流式输出](/zh/model-studio/stream)，但调用时未启用流式输出。

**解决方案：** 请使用[流式输出](/zh/model-studio/stream)方式调用模型。

### This model does not support enable\_search.

**原因：** 当前模型不支持[联网搜索](/zh/model-studio/web-search)能力，但指定了`enable_search`参数为`true`。

**解决方案：** 请调用支持联网搜索能力的模型。

### 暂时不支持当前设置的语种！

**原因：** 使用 Qwen-MT 模型时，传入的 `source_lang` 或 `target_lang` 格式错误，或不在[支持的语言](/zh/model-studio/machine-translation#038d2865bbydc)里。
...
**解决方案：** 请指定`messages`参数。如果已指定但仍报错，请参见[文本生成](/zh/model-studio/qwen-api-reference)API文档，检查其位置是否正确。

### 'messages' must contain the word 'json' in some form, to use 'response\_format' of type 'json\_object'.

**原因：** 使用[结构化输出](/zh/model-studio/qwen-structured-output)时，提示词中不包含 `json` 关键词。

**解决方案：** 在提示词中加入`json`（不区分大小写），如：“请以json格式输出”。

### Json mode response is not supported when enable\_thinking is true

**原因：** 使用[结构化输出](/zh/model-studio/qwen-structured-output)时开启了模型的思考模式。

**解决方案：** 请在使用结构化输出时，将`enable_thinking`设为`false`关闭思考模式。也可参见常见问题[思考模式模型如何结构化输出？](/zh/model-studio/qwen-structured-output#2f7ba877d3hle)
...
**解决方案：** 如需使用[结构化输出](/zh/model-studio/qwen-structured-output)功能，请将`response_format`参数设置为`{"type": "json_object"}`。
...
-   **使用正确的模型名称：**请对照[模型列表](/zh/model-studio/models)中的模型名称，检查输入的`model`是否正确。请勿混用开源社区的模型名与百炼模型ID，如应该使用`qwen3-235b-a22b-instruct-2507`，而非`Qwen/Qwen3-235B-A22B-Instruct-2507`。
...
**解决方案：** 参见[文本生成](/zh/model-studio/qwen-api-reference)API 文档，设置正确格式的`stop` 参数。

### Value error, batch size is invalid, it should not be larger than xxx.

**原因：** 调用 Embedding 模型时，文本数量超过模型上限。

**解决方案：** 参考[Embedding](/zh/model-studio/embedding)文档中模型的**批次大小**信息，控制传入文本的数量。

### Invalid file \[id:file-fe-\*\*\*\*\*\*\*\*\*\*\].

**原因**：提供的 file-id 无效。例如输入错误、使用了不属于当前阿里云账号的 file-id。

**解决方案**：通过[OpenAI兼容-File](/zh/model-studio/openai-file-interface)确认file-id是否有效，或重新[OpenAI兼容-File](/zh/model-studio/openai-file-interface)来获取新的file\_id后进行调用。
...
当报错信息中参数名称为`data_sources`时，通常是因为调用 CreateIndex 接口时未指定必传参数`SourceType`导致后续 SubmitIndexJob 接口报错。基于给定文档创建知识库时，此参数需传入`DATA_CENTER_FILE`；基于给定类目创建知识库时，此参数需传入`DATA_CENTER_CATEGORY`。详见[CreateIndex](/zh/model-studio/api-bailian-2023-12-29-createindex)文档。
...
**解决方案：** Qwen3-VL与Qwen2.5-VL系列模型需传入4-512张图片；其他模型需传入4-80张图片。详情可参见[图像与视频理解](/zh/model-studio/vision)。
...
> 压缩文件体积请参见[如何将图像或视频压缩到满足要求的大小？](/zh/model-studio/vision)
...
-   **调整传输方式**
    ...
    若传入公网 URL 方式失败，可参考[传入本地文件（Base64 编码或文件路径）](/zh/model-studio/vision#d987f8de5395x)切换为推荐的传入方式：
...
-   **放行**`DashScopeUserBot`
    ...
    若图片托管在您控制的服务器上，请在安全策略中放行 User-Agent 包含 `DashScopeUserBot` 的请求。也可改用[传入本地文件（Base64 编码或文件路径）](/zh/model-studio/vision#d987f8de5395x)的方式，避免服务端下载公网 URL。
...
-   **原因一： 模型名称与API 端点不匹配**：例如调用纯文本模型时使用了多模态接口，或者调用多模态模型时使用了纯文本接口。
    ...
    **解决方案：**
    1.  通过DashScope使用qwen3.7-plus、qwen3-vl-plus、qwen3.8-max、qwen3.8-flash等**多模态模型**时：需使用MultiModalConversation.call() 或multimodal-generation端点，具体参考[图像与视频理解](/zh/model-studio/vision)。从纯文本模型（如qwen3.7-max）切换为多模态模型（如qwen3.8-max）时，需将调用方式从Generation.call() 改为MultiModalConversation.call()，否则会返回url error。
    ...
    2.  通过DashScope使用qwen3-max、qwen-plus、deepseek-v4-pro等**纯文本模型**时，需使用Generation.call() 或 text-generation 端点，具体参考[概述](/zh/model-studio/text-generation)。
        
    3.  通过DashScope使用CosyVoice**声音复刻**接口时：该接口包含 `model` 与 `target_model` 参数：需将 `model` 设置为 `voice-enrollment`， `target_model` 设置为具体的 CosyVoice 模型，具体参考[CosyVoice声音复刻/设计API](/zh/model-studio/cosyvoice-clone-design-api)。
        
-   **原因二：DashScope SDK 版本过低：**调用图像/视频生成模型时，旧版SDK无法识别正确的服务端地址；
    
    **解决方案：**[升级 SDK 版本](/zh/model-studio/install-sdk)
...
**解决方案：** 详情参见[EMO 视频生成](/zh/model-studio/emo-api)。
...
**解决方案：** 详情参见[视频风格重绘](/zh/model-studio/video-style-transform-api-reference)。
...
**解决方案：** 详情参见[千问-图像编辑](/zh/model-studio/qwen-image-edit-api)。
...
**解决方案：** 即只能传入一条对话消息。详情参见[千问-图像编辑](/zh/model-studio/qwen-image-edit-api)。

### content length only support 2

**原因：** content数组长度仅支持为2。

**解决方案：** 即只能传入一组text和image。详情参见[千问-图像编辑](/zh/model-studio/qwen-image-edit-api)。
...
**解决方案：** 音频支持格式mp3, wav, aac，图片支持格式jpg, jpeg, png, bmp, webp。详情参见[LivePortrait 视频生成](/zh/model-studio/liveportrait-api)。
...
**解决方案：** 详情参见[AI试衣-图片分割](/zh/model-studio/aitryon-parsing-api)；若为画幅入参问题，可选"1:1"或"3:4"。

### The type or value of {parameter} is out of definition.

**原因：** 参数类型或值不符合要求。

**解决方案：** 详情参见[LivePortrait 视频生成](/zh/model-studio/liveportrait-api)。

### request timeout after 23 seconds.

**原因：** 超过23秒未向服务发送数据。该报错信息在使用[实时语音合成（Sambert）](/zh/model-studio/sambert-speech-synthesis)、[语音识别（Paraformer）](/zh/model-studio/paraformer-real-time-speech-recognition-api-reference)和[实时语音合成（Qwen-Audio-TTS/CosyVoice）](/zh/model-studio/cosyvoice-large-model-for-speech-synthesis)时产生。

**解决方案：** 请检查为什么长时间未向服务器发送数据。如果长时间（超过23秒）不向服务端发送消息，请及时结束任务。

### Please ensure input text is valid.

**原因：** 若您使用[实时语音合成（Qwen-Audio-TTS/CosyVoice）](/zh/model-studio/cosyvoice-large-model-for-speech-synthesis)，此错误通常是由于未发送待合成文本引起的。可能原因包括：参数遗漏（未为 `text` 参数赋值）或代码异常（导致对 `text` 参数的赋值失败）。
...
**原因：** 使用[实时语音合成（Qwen-Audio-TTS/CosyVoice）](/zh/model-studio/cosyvoice-large-model-for-speech-synthesis)，请求参数 `voice`（音色）不正确，或 `model`（模型）与 `voice`（音色）版本不匹配。

**解决方案：**

1.  **检查** `voice` **参数赋值**：
    
    -   如果使用的是默认音色，请对照[Python SDK](/zh/model-studio/cosyvoice-python-sdk)中的“voice参数”进行确认。
    -   如果使用的是声音复刻音色，请通过[CosyVoice声音复刻/设计API](/zh/model-studio/cosyvoice-clone-design-api)接口确认音色状态为“OK”，并确保音色归属账号与调用账号一致。
2.  **检查版本匹配**：v2模型只能使用v2的音色，v1模型只能使用v1的音色，两者不可混用。
    

### Request voice is invalid!

**原因：** 若您使用[实时语音合成（Qwen-Audio-TTS/CosyVoice）](/zh/model-studio/cosyvoice-large-model-for-speech-synthesis)，此错误通常是因为未设置音色。

**解决方案：** 请检查是否对`voice`参数赋值。若您使用[WebSocket API参考](/zh/model-studio/cosyvoice-websocket-api)，请参照API文档按照正确JSON格式配置参数。

### ref\_images\_url and obj\_or\_bg must be the same length.

**原因：** 使用[万相-视频编辑（2.1）](/zh/model-studio/legacy-wanx-vace-api-reference)的多图参考功能时，`ref_images_url`和`obj_or_bg`的数组长度不一致。
...
-   若使用 [Cline](/zh/model-studio/cline) ，请在设置界面单击`MODEL CONFIGURATION`，并勾选 **Enable R1 messages format**。
...
> 千问VL模型支持的Content Type为：image/bmp、image/bmp、image/icns、image/x-icon、image/jpeg、image/jp2、image/png、image/sgi、image/tiff、image/webp。详情可参见[千问VL模型支持的图像](/zh/model-studio/vision)。
...
**解决方案：** 请参考[Emoji 视频生成](/zh/model-studio/emoji-api)，传入`image_url`参数。

### Field required: driven\_id

**原因：** 缺少入参`driven_id`。

**解决方案：** 请参考[Emoji 视频生成](/zh/model-studio/emoji-api)，传入`driven_id`参数。

### Invalid ext\_bbox

**原因：** 输入`ext_bbox`参数无效。

**解决方案：** 请参考[Emoji 视频生成](/zh/model-studio/emoji-api)，传入正确的`ext_bbox`。

### Driven not exist: driven\_id

**原因：**输入`driven_id`不存在。

**解决方案：** 请参考[Emoji 视频生成](/zh/model-studio/emoji-api)，传入正确的`driven_id`。

### Text request limit violated, expected 1.

**原因：**在调用CosyVoice语音合成的[WebSocket API参考](/zh/model-studio/cosyvoice-websocket-api)时，将`enable_ssml`设为`true`后多次发送continue-task指令。
...
**解决方案：**请根据[限制与约束](/zh/model-studio/introduction-to-cosyvoice-ssml-markup-language)进行排查
...
-   **格式有误**或**语言有误**：仅以下模型支持 `instruction`，且规则各不相同：
    ...
    -   cosyvoice-v3-flash：
        ...
        -   须按固定格式编写
        -   仅支持中文 `instruction`
        -   不同音色支持的 `instruction` 不同，详情请参见[CosyVoice音色列表](/zh/model-studio/cosyvoice-voice-list)
...
**原因：** 使用[语音识别（Paraformer）](/zh/model-studio/paraformer-real-time-speech-recognition-api-reference)录音文件识别，待识别文件下载失败。
...
**解决方案：** 详情参见[VideoRetalk视频生成](/zh/model-studio/videoretalk-api)。
...
**解决方案：** 如果从海外发起调用，由于跨境网络不稳定，可能会导致下载资源超时。请将文件存储到国内的 [OSS](https://help.aliyun.com/zh/oss/user-guide/what-is-oss) 中，再发起模型调用。也可以使用[临时存储空间](/zh/model-studio/get-temporary-file-url)上传文件。
...
**原因：**[CosyVoice声音复刻](/zh/model-studio/voice-replica-1)时，上传的音频格式不符合模型要求。
...
-   “purpose”参数为“batch”时，文件不能超500MB。 请拆分并分批[上传文件](/zh/model-studio/openai-file-interface)。
...
-   [CosyVoice声音复刻](/zh/model-studio/voice-replica-1)更新、查询或删除接口调用时，对应音色不存在。
-   使用[定制热词（Paraformer）](/zh/model-studio/custom-hot-words)时，更新、查询或删除接口调用的热词资源不存在。

## 400- Throttling.AllocationQuota

### 您当前的配额为xxx

**原因：** [CosyVoice声音复刻](/zh/model-studio/voice-replica-1)音色数量已达限额。

**解决方案：** [删除](/zh/model-studio/cosyvoice-clone-design-api)部分音色。
...
**解决方案：** 请[删除](/zh/model-studio/qwen-tts-voice-cloning)一部分音色。
...
**原因：** 用于[CosyVoice声音复刻](/zh/model-studio/voice-replica-1)的音频有效时长过短。
...
**原因：** [CosyVoice声音复刻](/zh/model-studio/voice-replica-1)音频文件为静音或非静音长度过短。
...
**解决方案：** 请参见[输入图片要求](/zh/model-studio/aitryon-plus-api#7239603c85fye)。
...
-   **工具适配问题**：第三方工具未正确适配（如[Dify](/zh/model-studio/dify)最新版本插件不稳定导致报错，可尝试安装旧版本千问插件；旧版本[Cline](/zh/model-studio/cline)调用模型时**API Provider**选择了**Alibaba Qwen**，应选择**OpenAI兼容**）。
...
**解决方案：** 请重新调用[文件上传凭证接口](/zh/model-studio/get-temporary-file-url#32db94982cllx)生成新凭证。

### Access denied.

**原因：** 无权访问此模型。可能因该模型需申请权限、模型免费额度已耗尽且不支持付费使用（如 `deepseek-r1-distill-llama-70b`），或该模型已下线。

**解决方案：** 若该模型已下线，请在[模型下线机制说明](/zh/model-studio/model-depreciation)中确认其下线状态，并改用该文档推荐的替代模型。
...
**解决方案：** 请前往阿里云百炼控制台，在**我的订阅**的 API Key 区域获取 Token Plan 专属 API Key 与专属 Base URL，并确认同时更新了两者，具体配置方法请参见[快速开始](/zh/model-studio/token-plan-team-quickstart)。若 API Key 与 Base URL 不匹配（例如专属 API Key 搭配通用 Base URL），将返回 401 鉴权错误，请参见本文档 **401-InvalidApiKey/invalid\_api\_key**。
...
-   **调用标准模型**：使用子业务空间的API-KEY调用标准模型（例如`qwen-plus`）时，子业务空间需具备该模型的调用权限。详见[模型调用授权](/zh/model-studio/use-workspace)。
...
-   如果调用子业务空间的应用，请参考[业务空间](/zh/model-studio/call-rag-application-through-api-old#78c7ddbcca64f)。
-   如果调用子业务空间的模型，请参考[子业务空间的模型调用](/zh/model-studio/model-calling-in-sub-workspace)。
...
-   前往[模型下线机制说明](/zh/model-studio/model-depreciation)确认所调用的模型是否已下线。
...
**原因二**：用户开启了[免费额度用完即停](/zh/model-studio/new-free-quota#d1cb80ac11i92)，且免费额度耗尽后发起请求。

> 控制台显示的免费额度为分钟级更新（需手动刷新页面）。

**解决方案**：

-   如需继续以付费方式调用，可随时关闭[免费额度用完即停](/zh/model-studio/new-free-quota#d1cb80ac11i92)开关，无需等到免费额度用完；关闭后即可按量付费继续调用。
-   若在使用 Coding Plan 时遇到此问题，通常是配置错误所致。Coding Plan 需要配置专属的 Base URL 和 [API Key](https://bailian.console.aliyun.com/cn-beijing/?tab=model#/efm/coding_plan)，详情请参见[Coding Plan快速开始](/zh/model-studio/coding-plan-quickstart)。
...
**解决方案：** 请参考[OpenAI兼容-Batch（文件输入）](/zh/model-studio/batch-interfaces-compatible-with-openai)，确认支持 Batch调用的模型及其正确名称。
...
-   请对照[模型列表](/zh/model-studio/models)中的模型名称，检查您输入的模型名称（参数`model`的取值）是否正确。
...
-   如需更高调用频率，可参考[限流](/zh/model-studio/rate-limit)申请提额。
...
**解决方案：** 请参考[限流](/zh/model-studio/rate-limit)，控制调用频率。
...
**解决方案：** 前往[限流](/zh/model-studio/rate-limit)文档查看模型限流条件并调整调用策略；如果默认额度无法满足业务需求，可在百炼控制台的[**限流提额**](/zh/model-studio/rate-limit#h2_title_temp_limit_raise)中提升模型的临时限流额度（TPM）。

> 可参考[FAQ](/zh/model-studio/rate-limit#d1688da3e3egs)避免触发限流。
...
**解决方案：** 使用其它模型替换，例如：千问Audio模型额度耗尽，可使用[非实时（Qwen-Omni）](/zh/model-studio/qwen-omni)模型。
...
**原因**：[CosyVoice声音复刻](/zh/model-studio/voice-replica-1)音频文件解码失败。
...
**原因：** [CosyVoice声音复刻](/zh/model-studio/voice-replica-1)音频文件大小超限。
...
**原因：** [CosyVoice声音复刻](/zh/model-studio/voice-replica-1)音频文件采样率不支持。
...
-   如果您使用[（Qwen-Omni）模型](/zh/model-studio/qwen-omni)，需要使用流式输出方式。
    
-   如果您使用[CosyVoice声音复刻](/zh/model-studio/voice-replica-1)，则可能的原因是：
    
    -   音频文件不规范，比如声音本身有问题，有杂音或者声音忽高忽低。请参见[录音操作指南](/zh/model-studio/recording-guide)录音后重试。
    -   录音文件URL无法访问，请按照[CosyVoice声音复刻/设计API](/zh/model-studio/cosyvoice-clone-design-api)中的说明操作后重试。
...
使用[CosyVoice声音复刻](/zh/model-studio/voice-replica-1)：

-   **原因：**音频文件不规范，比如声音本身有问题，有杂音或者声音忽高忽低。
    
    **解决方案**：请参见[录音操作指南](/zh/model-studio/recording-guide)录音后重试。
    
-   **原因**：录音文件URL无法访问，
    
    **解决方案**：请按照[CosyVoice声音复刻/设计API](/zh/model-studio/cosyvoice-clone-design-api)中的说明操作后重试。
...
**原因：**使用[CosyVoice声音复刻](/zh/model-studio/voice-replica-1)时，音频文件不规范，未包含有效人声或声音不清晰，杂音大。
...
**解决方案：** 请检查`voice`参数，确保指定了正确的音色名称。可用音色请参见[实时语音合成（Qwen-Audio-TTS/CosyVoice）](/zh/model-studio/cosyvoice-large-model-for-speech-synthesis)。
...
**解决方案：** 请稍后重试。若您使用[调用百炼应用](/zh/model-studio/spring-ai-alibaba-integrate-llm-application)，请参照示例代码或说明文档，查看是否代码编写有误，若依然无法确定问题，加入[Spring AI Alibaba官网](https://java2ai.com/?spm=4347728f.638c0b20.0.0.23f87982NTcSMy)最下方提供的DING群，联系开发人员进行定位。
...
-   通过流式输出方式发起请求，具体操作请参见[流式输出](/zh/model-studio/stream)。
-   将请求参数`heartbeat`设为`true`或及时结束识别任务。
-   调用图像模型时，可尝试降低图片分辨率、简化编辑需求，或稍后重试。

> 调用千问模型时，响应体中会将已生成的内容返回，不再报超时错误。详情请参见[文本生成](/zh/model-studio/text-generation#11241147efwpm)。
...
**解决方案：** 具体配置API Key的方法，请参见[配置API Key到环境变量](/zh/model-studio/configure-api-key-through-environment-variables)。
...
-   **通过环境变量传入 API Key 来源（推荐）**
    ...
    将`DASHSCOPE_API_KEY`设为环境变量（参见[配置API Key到环境变量](/zh/model-studio/configure-api-key-through-environment-variables)），初始化`client`时，通过`os.getenv`读取：
...
**原因：** 若您使用[实时语音合成（Qwen-Audio-TTS/CosyVoice）](/zh/model-studio/cosyvoice-large-model-for-speech-synthesis)，出现该问题的原因是DashScope SDK版本过低。

**解决方案：** 请[安装最新版 DashScope SDK](/zh/model-studio/install-sdk)。
...
**解决方案：** 您可以重启客户端或IDE后重试。更多情况请参考[常见问题](/zh/model-studio/configure-api-key-through-environment-variables)。
...
**原因**：使用[实时语音合成（Qwen-Audio-TTS/CosyVoice）](/zh/model-studio/cosyvoice-large-model-for-speech-synthesis)时未发送待合成文本。
...
**解决方案：** 请根据[OpenAI兼容-Batch（文件输入）](/zh/model-studio/batch-interfaces-compatible-with-openai)检查您的输入文件。
...
**解决方案：** 请根据[OpenAI兼容-Batch（文件输入）](/zh/model-studio/batch-interfaces-compatible-with-openai)检查您的输入文件，确保所有请求 ID 不重复。

### Upload file capacity exceed limit. / Upload file number exceed limit.

**原因：** 上传文件失败，当前阿里云账号下的阿里云百炼存储空间已满或接近满额。

**解决方案：** 可以通过[OpenAI兼容-File](/zh/model-studio/openai-file-interface)接口删除不需要的文件以释放空间。当前存储空间支持最大文件数为10000个，总量不超过100 GB。
...
-   请对照[模型列表](/zh/model-studio/models)中的模型名称，检查您输入的模型名称（参数 `model` 的取值）是否正确。实时模型的名称含 `-realtime` 等后缀，不可省略或改写。
