...
-   已[获取与配置 API Key](/zh/model-studio/get-api-key)并将其[配置到环境变量](/zh/model-studio/configure-api-key-through-environment-variables)
-   已安装符合版本要求的DashScope Java SDK，建议[安装最新版](/zh/model-studio/install-sdk)，SDK版本需≥1.25.17
...
-   使用 SSML 时，需同时将 `enable_ssml` 设置为 `True`。支持的 SSML 标签及用法，请参见[SSML 与 LaTeX](/zh/model-studio/ssml-latex-user-guide)。
-   使用 LaTeX 时，将待合成文本替换为 LaTeX 格式即可，无需额外配置。支持的 LaTeX 语法及用法，请参见[LaTeX 公式转语音](/zh/model-studio/latex-capability-support-description)。
...
-   声音复刻音色：如何创建音色请参见[CosyVoice声音复刻/设计API](/zh/model-studio/cosyvoice-clone-design-api)
-   声音设计音色：如何创建音色请参见[CosyVoice声音复刻/设计API](/zh/model-studio/cosyvoice-clone-design-api)
...
是否开启SSML功能。当`text`使用SSML格式时，需设为`True`。默认为`False`。支持的SSML标签及用法，请参考[SSML 与 LaTeX](/zh/model-studio/ssml-latex-user-guide)。SSML 的使用限制（支持的模型、音色和接口），请参见[使用限制](/zh/model-studio/ssml-latex-user-guide#sl01_constraint_h3)。
...
具体用法请参见[非实时语音合成](/zh/model-studio/non-realtime-tts-user-guide)。
...
**重要**不同模型版本需使用对应版本的音色。例如`cosyvoice-v3-flash`和`cosyvoice-v3-plus`使用`longanhuan`等音色，`cosyvoice-v2`使用`longxiaochun_v2`等音色。更换模型时请同步更换为对应版本的音色。此外，每个音色支持的语言不同，合成非中文语言时，需选择支持对应语言的音色。具体的模型与音色对应关系，请参见[CosyVoice音色列表](/zh/model-studio/cosyvoice-voice-list)。
