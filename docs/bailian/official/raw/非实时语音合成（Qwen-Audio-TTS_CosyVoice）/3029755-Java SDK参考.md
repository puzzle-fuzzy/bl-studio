...
-   已安装符合版本要求的DashScope Java SDK，建议[安装最新版](/zh/model-studio/install-sdk)，SDK版本需≥2.22.15
...
[HttpSpeechSynthesisParam](/zh/model-studio/cosyvoice-tts-java-sdk#h2-cv3a4b5c)
...
是否开启SSML功能。设置为`true`时，`text`参数需传入SSML格式文本。支持的SSML标签及用法，请参见[SSML 与 LaTeX](/zh/model-studio/ssml-latex-user-guide)。SSML 的使用限制（支持的模型、音色和接口），请参见[使用限制](/zh/model-studio/ssml-latex-user-guide#sl01_constraint_h3)。
...
**重要**不同模型版本需使用对应版本的音色。例如`cosyvoice-v3-flash`和`cosyvoice-v3-plus`使用`longanyang`等音色，`cosyvoice-v2`使用`longxiaochun_v2`等音色。更换模型时请同步更换为对应版本的音色。此外，每个音色支持的语言不同，合成非中文语言时，需选择支持对应语言的音色。具体的模型与音色对应关系，请参见[CosyVoice音色列表](/zh/model-studio/cosyvoice-voice-list)。
