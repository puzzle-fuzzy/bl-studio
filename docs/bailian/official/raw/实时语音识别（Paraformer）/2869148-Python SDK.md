...
✅ 参见[定制热词](/zh/model-studio/custom-hot-words)
...
✅ 参见[Paraformer语音识别热词定制与管理](/zh/model-studio/paraformer-asr-phrase-manager)
...
-   必须关闭语义断句（可通过[请求参数](/zh/model-studio/paraformer-real-time-speech-recognition-python-sdk#555007db2033f)`semantic_punctuation_enabled`控制）。语义断句默认为关闭状态。
-   只有在[识别结果（RecognitionResult）](/zh/model-studio/paraformer-real-time-speech-recognition-python-sdk#bc3e1a43d6hhy)的`is_sentence_end`方法返回结果为`True`时才显示情感识别结果。

情感识别结果获取方式：通过[单句信息（Sentence）](/zh/model-studio/paraformer-real-time-speech-recognition-python-sdk#f28f50b035qtn)的`emo_tag`和`emo_confidence`字段分别获取当前句子的情感和情感置信度。
...
[Recognition类](/zh/model-studio/paraformer-real-time-speech-recognition-python-sdk#d6bc1f133f871)提供了非流式调用和双向流式调用接口。请根据实际需求选择合适的调用方式：
...
实例化[Recognition类](/zh/model-studio/paraformer-real-time-speech-recognition-python-sdk#d6bc1f133f871)绑定[请求参数](/zh/model-studio/paraformer-real-time-speech-recognition-python-sdk#555007db2033f)，调用`call`进行识别/翻译并最终获取[识别结果（RecognitionResult）](/zh/model-studio/paraformer-real-time-speech-recognition-python-sdk#bc3e1a43d6hhy)。
...
在流式回调（`on_event`）中，`result.get_sentence()`返回**单句信息**（`Dict[str, Any]`），可直接使用`sentence['text']`获取识别文本。两种调用模式的返回值类型不同，详见[识别结果（RecognitionResult）](/zh/model-studio/paraformer-real-time-speech-recognition-python-sdk#bc3e1a43d6hhy)中的`get_sentence`方法说明。
...
1.  启动流式语音识别
    ...
    实例化[Recognition类](/zh/model-studio/paraformer-real-time-speech-recognition-python-sdk#d6bc1f133f871)绑定[请求参数](/zh/model-studio/paraformer-real-time-speech-recognition-python-sdk#555007db2033f)和[回调接口（RecognitionCallback）](/zh/model-studio/paraformer-real-time-speech-recognition-python-sdk#85d698b9f9g8s)，调用`start`方法启动流式语音识别。
    
2.  流式传输
    
    循环调用[Recognition类](/zh/model-studio/paraformer-real-time-speech-recognition-python-sdk#d6bc1f133f871)的`send_audio_frame`方法，将从本地文件或设备（如麦克风）读取的二进制音频流分段发送至服务端。
    
    在发送音频数据的过程中，服务端会通过[回调接口（RecognitionCallback）](/zh/model-studio/paraformer-real-time-speech-recognition-python-sdk#85d698b9f9g8s)的`on_event`方法，将识别结果实时返回给客户端。
    
    建议每次发送的音频时长约为100毫秒，数据大小保持在1KB至16KB之间。
    
3.  结束处理
    
    调用[Recognition类](/zh/model-studio/paraformer-real-time-speech-recognition-python-sdk#d6bc1f133f871)的`stop`方法结束语音识别。
    
    该方法会阻塞当前线程，直到[回调接口（RecognitionCallback）](/zh/model-studio/paraformer-real-time-speech-recognition-python-sdk#85d698b9f9g8s)的`on_complete`或者`on_error`回调触发后才会释放线程阻塞。
...
请求参数通过[Recognition类](/zh/model-studio/paraformer-real-time-speech-recognition-python-sdk#d6bc1f133f871)的构造方法（*init*）进行设置。
...
用于实时语音识别的模型（参见[模型列表](/zh/model-studio/paraformer-real-time-speech-recognition-python-sdk#1e173d999c1th)）。
...
在本次语音识别中，将应用与该热词ID对应的热词信息。具体使用方法请参见[定制热词](/zh/model-studio/custom-hot-words)。
...
在本次语音识别中，将应用与该热词ID对应的热词信息。具体使用方法请参见[Paraformer语音识别热词定制与管理](/zh/model-studio/paraformer-asr-phrase-manager)。
...
该参数仅对支持多语言的模型生效（参见[模型列表](/zh/model-studio/paraformer-real-time-speech-recognition-python-sdk#1e173d999c1th)）。
...
[回调接口（RecognitionCallback）](/zh/model-studio/paraformer-real-time-speech-recognition-python-sdk#85d698b9f9g8s)。
...
识别结果通过[回调接口（RecognitionCallback）](/zh/model-studio/paraformer-real-time-speech-recognition-python-sdk#85d698b9f9g8s)的on\_event方法获取。
...
[双向流式调用](/zh/model-studio/paraformer-real-time-speech-recognition-python-sdk#9d1e5f6852jr8)时，服务端会通过回调的方式，将关键流程信息和数据返回给客户端。您需要实现回调方法，处理服务端返回的信息或者数据。
...
[字时间戳信息（Word）](/zh/model-studio/paraformer-real-time-speech-recognition-python-sdk#b55a7391caxxe)的list集合
...
-   直接传入本地文件路径：此种方式在最终识别结束后获取完整识别结果，不适合即时反馈的场景。
    ...
    参见[非流式调用](/zh/model-studio/paraformer-real-time-speech-recognition-python-sdk#8341058094tc3)，在[Recognition类](/zh/model-studio/paraformer-real-time-speech-recognition-python-sdk#d6bc1f133f871)的`call`方法中传入文件路径对录音文件直接进行识别。
    
-   将本地文件转成二进制流进行识别：此种方式一边识别文件一边流式获取识别结果，适合即时反馈的场景。
    
    参见[双向流式调用](/zh/model-studio/paraformer-real-time-speech-recognition-python-sdk#9d1e5f6852jr8)，通过[Recognition类](/zh/model-studio/paraformer-real-time-speech-recognition-python-sdk#d6bc1f133f871)的`send_audio_frame`方法向服务端发送二进制流对其进行识别。
