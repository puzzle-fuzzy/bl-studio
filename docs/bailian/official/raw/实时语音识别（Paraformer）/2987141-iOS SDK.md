...
1.  **获取API Key：**[获取与配置 API Key](/zh/model-studio/get-api-key)
    ...
    **说明**当需要为第三方应用或用户提供临时访问权限，或者希望严格控制敏感数据访问、删除等高风险操作时，建议使用[临时API Key](/zh/model-studio/generate-temporary-api-key)。临时API Key拥有固定的60秒有效期，过期后需重新获取。
...
1.  初始化 SDK
2.  按业务需求设置参数：通过[nui\_initialize](/zh/model-studio/ios-sdk-for-paraformer-real-time-service#05eab5125e2pm)接口设置[连接与控制参数](/zh/model-studio/ios-sdk-for-paraformer-real-time-service#57acf5ecc1w8j)；通过[nui\_set\_params](/zh/model-studio/ios-sdk-for-paraformer-real-time-service#763672f3f8dgw)接口设置[语音识别效果参数](/zh/model-studio/ios-sdk-for-paraformer-real-time-service#d20cce9518kla)。
3.  调用[nui\_dialog\_start](/zh/model-studio/ios-sdk-for-paraformer-real-time-service#8fe6ea298apzu)启动识别流程。
4.  在[onNuiAudioStateChanged](/zh/model-studio/ios-sdk-for-paraformer-real-time-service#bc71fe2545pfy)回调中，根据音频状态开启录音设备。
5.  在[onNuiNeedAudioData](/zh/model-studio/ios-sdk-for-paraformer-real-time-service#46174611d31qf)回调中持续提供录音数据。
6.  在[onNuiEventCallback](/zh/model-studio/ios-sdk-for-paraformer-real-time-service#163c1ef871tqt)回调中监听事件并获取语音识别结果。
7.  调用[nui\_dialog\_cancel](/zh/model-studio/ios-sdk-for-paraformer-real-time-service#156934a01bzjc)停止识别，并通过监听EVENT\_TRANSCRIBER\_COMPLETE事件确认识别已结束。
8.  当识别功能不再使用时，调用[nui\_release](/zh/model-studio/ios-sdk-for-paraformer-real-time-service#6c2931e9ae3eq)接口释放 SDK 资源。

## 请求参数

### 连接与控制参数

通过在[nui\_initialize](/zh/model-studio/ios-sdk-for-paraformer-real-time-service#05eab5125e2pm)接口的`parameters`参数中传入一个JSON字符串来配置。
...
-   **参数说明**
    ...
    **说明**
    ...
    此参数仅在调用[nui\_initialize](/zh/model-studio/ios-sdk-for-paraformer-real-time-service#05eab5125e2pm)接口时将`save_log`设为`YES`时生效。此时必须设置日志文件路径，否则将报错。
    ...
    此参数仅在调用[nui\_initialize](/zh/model-studio/ios-sdk-for-paraformer-real-time-service#05eab5125e2pm)接口时将`save_log`设为YES时生效。 同时，`debug_path`也必须被设置。
    ...
    此参数仅在调用[nui\_initialize](/zh/model-studio/ios-sdk-for-paraformer-real-time-service#05eab5125e2pm)接口时将`save_log`设为`YES`时生效。
    ...
    控制通过日志回调（[onNuiLogTrackCallback](/zh/model-studio/ios-sdk-for-paraformer-real-time-service#9c10968457gc6)）对外发送的日志内容的过滤级别。
    ...
    注意：`log_track_level`与`level`（通过[nui\_initialize](/zh/model-studio/ios-sdk-for-paraformer-real-time-service#05eab5125e2pm)接口设置）共同决定最终回调的日志。一条日志的级别数值必须同时大于或等于`log_track_level`和`level`的值，才会被回调。例如，`log_track_level`设为2 (INFO)，`level`设为3 (WARNING)，则只有WARNING及以上级别（数值>=3）的日志才会被回调。
...
通过在[nui\_set\_params](/zh/model-studio/ios-sdk-for-paraformer-real-time-service#763672f3f8dgw)接口的`params`参数中传入一个JSON字符串来配置。
...
-   **参数说明**
    ...
    **说明**
    ...
    JSON字符串，包含鉴权、连接和调试参数。参见[连接与控制参数](/zh/model-studio/ios-sdk-for-paraformer-real-time-service#57acf5ecc1w8j)。
    ...
    是否保存本地日志。若为`YES`，须在[连接与控制参数](/zh/model-studio/ios-sdk-for-paraformer-real-time-service#57acf5ecc1w8j)通过`debug_path`指定路径，并可通过`max_log_file_size`设置文件大小。
...
-   **参数说明**
    ...
    **说明**
    ...
    [语音识别效果参数](/zh/model-studio/ios-sdk-for-paraformer-real-time-service#d20cce9518kla)。
...
-   **参数说明**
    ...
    当[连接与控制参数](/zh/model-studio/ios-sdk-for-paraformer-real-time-service#57acf5ecc1w8j)的`apikey`参数对应的[临时API Key](/zh/model-studio/generate-temporary-api-key)过期时，可在此处进行更新。
