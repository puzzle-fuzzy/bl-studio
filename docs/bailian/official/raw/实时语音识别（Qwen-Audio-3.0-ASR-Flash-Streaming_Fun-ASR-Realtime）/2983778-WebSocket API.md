...
**重要**URL 必须使用 `wss://` 协议，且固定不变。Authorization 在请求头中设置（参见[请求头](/zh/model-studio/fun-asr-realtime-websocket-api)）。
...
客户端事件和服务端事件的详细说明，请参见[客户端事件](/zh/model-studio/fun-asr-client-events)和[服务端事件](/zh/model-studio/fun-asr-server-events)。
