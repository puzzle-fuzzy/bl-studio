...
在[密钥管理（北京）](https://bailian.console.aliyun.com/?tab=model#/api-key)、[密钥管理（新加坡）](https://modelstudio.console.aliyun.com/?tab=model#/api-key)或[密钥管理（弗吉尼亚）](https://modelstudio.console.aliyun.com/us-east-1?tab=model#/api-key)页面创建永久API Key，并将其配置为环境变量 `DASHSCOPE_API_KEY`。配置方法请参见[配置API Key 到环境变量](/zh/model-studio/configure-api-key-through-environment-variables)。
...
错误码。更多错误原因及解决方法，请前往[错误码](/zh/model-studio/error-code)页面查询。
