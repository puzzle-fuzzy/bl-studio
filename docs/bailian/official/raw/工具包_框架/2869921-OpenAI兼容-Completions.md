...
当前支持 [Qwen Coder](/zh/model-studio/qwen-coder) 部分模型：

qwen-coder-turbo

## 前提条件

您需要已[获取与配置 API Key](/zh/model-studio/get-api-key)并[配置API Key到环境变量](/zh/model-studio/configure-api-key-through-environment-variables)。如果通过 OpenAI SDK 调用，需要[安装SDK](/zh/model-studio/install-sdk)。
