...
-   测试文件需满足[输入文件要求](/zh/model-studio/batch-interfaces-compatible-with-openai#0214fa4f9dxb3)，且文件大小不超过 **1 MB**，行数不超过**100行**。
-   并发限制：最大并行任务数 **2 个**。
-   费用：测试模型不产生模型推理费用。
...
-   失败请求详情（`error_file_id`）：包含处理失败的请求行信息和错误原因，可参考[错误码](/zh/model-studio/batch-interfaces-compatible-with-openai#97e145aeabbwf)进行排查。

## 具体流程

Batch API使用流程分为四步：上传文件、创建任务、查询任务状态、下载结果。

**说明**如果数据已存储在阿里云OSS中，可跳过文件上传步骤，直接在创建 Batch 任务时使用OSS文件路径。详情请参见[使用OSS文件创建 Batch 任务](/zh/model-studio/batch-interfaces-compatible-with-openai#f667257850olj)。
...
-   [准备与上传文件](/zh/model-studio/batch-interfaces-compatible-with-openai)接口返回的文件ID，如`file-batch-xxx`；
    
-   [使用OSS文件创建 Batch 任务](/zh/model-studio/batch-interfaces-compatible-with-openai#f667257850olj)。
...
错误行信息和错误原因，可参考[错误码](/zh/model-studio/batch-interfaces-compatible-with-openai#97e145aeabbwf)进行排查。
...
-   **计费单价：** 所有成功请求的输入和输出Token，单价均为对应模型实时推理价格的**50%** ，具体请参见[模型列表](/zh/model-studio/models)。
...
-   批量推理为独立计费项，支持AI 通用型节省计划，但不支持[预付费](https://common-buy.aliyun.com/?commodityCode=sfm_llminference_spn_public_cn)（节省计划）、新人免费额度等优惠，以及[上下文缓存](/zh/model-studio/context-cache)等功能。
