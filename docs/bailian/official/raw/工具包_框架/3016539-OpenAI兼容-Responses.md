...
`qwen3.8-max 、 qwen3.7-max 、 qwen3.7-max-2026-05-20 、 qwen3.7-max-2026-06-08 、 qwen3.7-plus 、 qwen3.7-plus-2026-05-26 、 qwen3.6-plus 、 qwen3.6-plus-2026-04-02 、 qwen3.5-plus 、 qwen3.5-plus-2026-02-15 、 qwen3.7-flash 、 qwen3.7-flash-2026-07-15 、 qwen3.6-flash 、 qwen3.6-flash-2026-04-16 、 qwen3.5-flash 、 qwen3.5-flash-2026-02-23 、 qwen3.6-35b-a3b 、 qwen3.5-397b-a17b 、 qwen3.5-122b-a10b 、 qwen3.5-27b 、 qwen3.5-35b-a3b`

#### 德国（法兰克福）

**全球部署范围**

`qwen3.8-max`、`qwen3.7-max`、`qwen3.7-max-2026-05-20`、`qwen3.7-max-2026-06-08`、`qwen3.7-plus`、`qwen3.7-plus-2026-05-26`、`qwen3.7-flash`、`qwen3.7-flash-2026-07-15`、`qwen3.5-397b-a17b`、`qwen3.5-122b-a10b`、`qwen3.5-35b-a3b`、`qwen3.5-27b`
...
`qwen3.7-plus`、`qwen3.7-plus-2026-05-26`、`qwen3.8-max`、`qwen3.7-max`、`qwen3.7-max-2026-05-20`、`qwen3.6-plus`、`qwen3.6-plus-2026-04-02`、`qwen3.7-flash`、`qwen3.7-flash-2026-07-15`、`qwen3.6-flash`、`qwen3.6-flash-2026-04-16`
...
开启内置工具可在处理复杂任务时获得更佳效果，当前网页抓取与代码解释器工具限时免费，支持的工具请参见[工具调用](/zh/model-studio/tool-calls)。
...
-   启用 Session 缓存：
    ...
    -   模型支持显式缓存：使用显式缓存，计费和相关约束参照[显式缓存](/zh/model-studio/context-cache#825f201c5fy6o)。
    -   模型不支持显式缓存但支持隐式缓存：使用隐式缓存，计费和相关约束参照[隐式缓存](/zh/model-studio/context-cache#2317ea09cfxok)。
...
<table><colgroup><col style="width:50%"><col style="width:50%"></colgroup><tbody><tr><td><pre data-tag="codeblock" id="code-block-6" outputclass="language-json" code-type="xCode" class="pre codeblock language-json"><code># Chat Completions 响应
{
...
</code></pre></td><td><pre data-tag="codeblock" id="code-block-7" outputclass="language-json" code-type="xCode" class="pre codeblock language-json"><code># Responses API 响应
{
...
<table><colgroup><col style="width:50%"><col style="width:50%"></colgroup><tbody><tr><td><pre data-tag="codeblock" id="code-block-8" outputclass="language-python" code-type="xCode" class="pre codeblock language-python"><code># Chat Completions - 需要手动管理消息历史
messages = [
    {"role": "system", "content": "你是一个有帮助的助手。"},
...
</code></pre></td><td><pre data-tag="codeblock" id="code-block-9" outputclass="language-python" code-type="xCode" class="pre codeblock language-python"><code># Responses API - 使用 previous_response_id 自动关联
...
<table><colgroup><col style="width:50%"><col style="width:50%"></colgroup><tbody><tr><td><pre data-tag="codeblock" id="code-block-10" outputclass="language-javascript" code-type="xCode" class="pre codeblock language-javascript"><code>// Chat Completions - 需要手动管理消息历史
let messages = [
    { role: "system", content: "你是一个有帮助的助手。" },
    { role: "user", content: "法国的首都是哪里？" }
];
...
</code></pre></td><td><pre data-tag="codeblock" id="code-block-11" outputclass="language-javascript" code-type="xCode" class="pre codeblock language-javascript"><code>// Responses API - 使用 previous_response_id 自动关联
...
Responses API 内置了多种工具，无需自行实现。只需在 `tools` 参数中指定即可，当前代码解释器与网页抓取工具限时免费，详情请参见[工具调用](/zh/model-studio/tool-calls)。

#### Python

<table><colgroup><col style="width:50%"><col style="width:50%"></colgroup><tbody><tr><td><pre data-tag="codeblock" id="code-block-12" outputclass="language-python" code-type="xCode" class="pre codeblock language-python"><code># Chat Completions - 需要自己实现工具函数
...
</code></pre></td><td><pre data-tag="codeblock" id="code-block-13" outputclass="language-python" code-type="xCode" class="pre codeblock language-python"><code># Responses API - 直接使用内置工具
response = client.responses.create(
    model="qwen3.8-max",
    input="法国现任总统是谁？",
...
<table><colgroup><col style="width:50%"><col style="width:50%"></colgroup><tbody><tr><td><pre data-tag="codeblock" id="code-block-14" outputclass="language-javascript" code-type="xCode" class="pre codeblock language-javascript"><code>// Chat Completions - 需要自己实现工具函数
...
</code></pre></td><td><pre data-tag="codeblock" id="code-block-15" outputclass="language-javascript" code-type="xCode" class="pre codeblock language-javascript"><code>// Responses API - 直接使用内置工具
const response = await client.responses.create({
    model: "qwen3.8-max",
    input: "法国现任总统是谁？",
    tools: [{ type: "web_search" }]  // 直接启用网络搜索
...
<table><colgroup><col style="width:50%"><col style="width:50%"></colgroup><tbody><tr><td><pre data-tag="codeblock" id="code-block-16" outputclass="language-bash" code-type="xCode" class="pre codeblock language-bash"><code># Chat Completions - 需要自己实现工具
# 这里展示调用外部搜索 API 的示例
curl https://api.example.com/search \
  -G \
  --data-urlencode "q=法国现任总统" \
  --data-urlencode "key=$SEARCH_API_KEY"
</code></pre></td><td><pre data-tag="codeblock" id="code-block-17" outputclass="language-bash" code-type="xCode" class="pre codeblock language-bash"><code># Responses API - 直接使用内置工具
curl -X POST https://{WorkspaceId}.cn-beijing.maas.aliyuncs.com/compatible-mode/v1/responses \
-H "Authorization: Bearer $DASHSCOPE_API_KEY" \
-H "Content-Type: application/json" \
-d '{
...
-   [创建响应](/zh/model-studio/qwen-api-via-openai-responses)
-   [获取与配置 API Key](/zh/model-studio/get-api-key)
-   [配置API Key到环境变量](/zh/model-studio/configure-api-key-through-environment-variables)
