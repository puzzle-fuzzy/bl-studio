...
-   base\_url：
    ...
    -   华北2（北京）地域：https://{WorkspaceId}.cn-beijing.maas.aliyuncs.com/compatible-mode/v1调用时请将 `WorkspaceId` 替换为真实的[业务空间 ID](/zh/model-studio/obtain-the-app-id-and-workspace-id#732535cfc959h)。
...
-   api\_key：替换为[阿里云百炼API Key](/zh/model-studio/get-api-key)
...
本章节提供Python（OpenAI SDK与LangChain\_OpenAI SDK）和cURL（HTTP接口）的流式调用示例，更多编程语言或输入方式示例请参考：[视觉理解请求示例](/zh/model-studio/vision)。

> QVQ模型仅支持流式输出，具体使用方法请参见[视觉推理](/zh/model-studio/visual-reasoning)。
...
-   您需要开通阿里云百炼模型服务并获得API-KEY，详情请参考：[获取与配置 API Key](/zh/model-studio/get-api-key)。
-   我们推荐您将API-KEY配置到环境变量中以降低API-KEY的泄露风险，详情可参考[配置API Key到环境变量](/zh/model-studio/configure-api-key-through-environment-variables)。您也可以在代码中配置API-KEY，**但是泄露风险会提高**。
...
> 以下示例不适用QVQ模型，QVQ调用方法请参见[视觉推理](/zh/model-studio/visual-reasoning)。
...
关于输入参数的配置，可以参考[输入参数配置](/zh/model-studio/compatibility-of-openai-with-dashscope#d553cbbee6mxk)，相关参数在ChatOpenAI对象中定义。
...
-   您需要开通阿里云百炼模型服务并获得API-KEY，详情请参考：[获取与配置 API Key](/zh/model-studio/get-api-key)。
-   我们推荐您将API-KEY配置到环境变量中以降低API-KEY的泄露风险，配置方法可参考[配置API Key到环境变量](/zh/model-studio/configure-api-key-through-environment-variables)。您也可以在代码中配置API-KEY，**但是泄露风险会提高**。
...
输入参数的详情请参考[输入参数配置](/zh/model-studio/compatibility-of-openai-with-dashscope#d553cbbee6mxk)。
...
相关状态错误码信息参考：[状态码说明](/zh/model-studio/compatibility-of-openai-with-dashscope#8dd39ae94bygm)。
