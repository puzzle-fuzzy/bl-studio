...
服务端响应事件（JSON格式），包含`header`（请求信息）和`payload`（输出信息）。其中`payload.output`包含事件类型、原始文本等信息，详见[on\_event消息中的output字段](/zh/model-studio/cosyvoice-python-sdk#h2-py-output-info)。
