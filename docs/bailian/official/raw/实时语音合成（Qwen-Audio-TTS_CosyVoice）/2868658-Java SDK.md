...
设置[扩展参数](/zh/model-studio/cosyvoice-java-sdk#8135356d13pvn)。
...
[SpeechSynthesisResult](/zh/model-studio/cosyvoice-java-sdk#h3-8k2m9p5r)
...
**返回值**：`com.google.gson.JsonObject`，合成事件的[输出信息](/zh/model-studio/cosyvoice-java-sdk#h4-output-info)，包含事件类型和文本内容。需要SDK版本 >= 2.22.0。
...
**返回值**：[Word](/zh/model-studio/cosyvoice-java-sdk#h4-word-ts)的`List`集合，批量获取字级别时间戳信息，可能为空。
...
**返回值**：[Phoneme](/zh/model-studio/cosyvoice-java-sdk#h4-phoneme-ts)的`List`集合，批量获取音素级别时间戳信息，可能为空。
...
在DashScope Java SDK中，采用了OkHttp3的连接池技术，以减少重复建立连接的开销。详情请参见[高并发最佳实践](/zh/model-studio/realtime-tts-user-guide#ug_hc_h3)。
