...
1.  [安装SDK](/zh/model-studio/install-sdk)，确保DashScope SDK版本不低于1.25.6。
2.  [获取与配置 API Key](/zh/model-studio/get-api-key)。
3.  了解[WebSocket API](/zh/model-studio/qwen-asr-realtime-interaction-process)。
...
-   以下参数通过`OmniRealtimeConversation`的构造方法设置。
    ...
    **说明**
    ...
    指定要使用的[模型](/zh/model-studio/real-time-speech-recognition-user-guide)名称。
    ...
    -   华北2（北京）地域：`wss://{WorkspaceId}.cn-beijing.maas.aliyuncs.com/api-ws/v1/realtime`。调用时请将`{WorkspaceId}`替换为真实的[Workspace ID](/zh/model-studio/obtain-the-app-id-and-workspace-id#732535cfc959h)。
        
    -   新加坡地域：`wss://{WorkspaceId}.ap-southeast-1.maas.aliyuncs.com/api-ws/v1/realtime`。调用时请将`{WorkspaceId}`替换为真实的[业务空间ID](/zh/model-studio/obtain-the-app-id-and-workspace-id#732535cfc959h)。
...
[session.created](/zh/model-studio/qwen-asr-realtime-server-events#2c04b24bc3wlo)

> 会话已创建

[session.updated](/zh/model-studio/qwen-asr-realtime-server-events#4d6ed9dd62vmj)
...
用于更新会话配置，建议在连接建立后首先调用该方法进行设置。若未调用该方法，系统将使用默认配置。只需关注[请求参数](/zh/model-studio/qwen-asr-realtime-python-sdk)中的涉及到的参数。
...
-   [请求参数](/zh/model-studio/qwen-asr-realtime-python-sdk)`enable_turn_detection`设为`True`，音频缓冲区用于检测语音，服务端决定何时提交。
-   [请求参数](/zh/model-studio/qwen-asr-realtime-python-sdk)`enable_turn_detection`设为`False`，客户端可以选择每个事件中放置多少音频量，最多放置 15 MiB。 例如，从客户端流式处理较小的数据块可以让 VAD 响应更迅速。
...
[input\_audio\_buffer.committed](/zh/model-studio/qwen-asr-realtime-server-events#1108a3764an0e)
...
**禁用场景：**[请求参数](/zh/model-studio/qwen-asr-realtime-python-sdk)`enable_turn_detection`设为`True`时。
...
[session.finished](/zh/model-studio/qwen-asr-realtime-server-events#6eaa77339djdv)
...
-   [VAD 模式（默认）](/zh/model-studio/qwen-asr-realtime-interaction-process#9b49887720jcw)下，发送完音频后调用该方法
-   [Manual 模式](/zh/model-studio/qwen-asr-realtime-interaction-process#ee09a3493fsuc)下，调用commit方法之后调用该方法
...
message：[服务端事件](/zh/model-studio/qwen-asr-realtime-server-events)
