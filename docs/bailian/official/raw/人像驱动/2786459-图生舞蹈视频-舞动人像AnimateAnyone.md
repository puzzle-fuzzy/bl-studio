...
-   调用时，需按模型名称调用对应模型，并请参照以下调用顺序：
    ...
    a. 调用“舞动人像AnimateAnyone-detect”模型确认输入的人物图像符合规范（可参考文档：[AnimateAnyone 图像检测](/zh/model-studio/animate-anyone-detect-api)）；
    
    b. 调用“舞动人像AnimateAnyone-template”模型输入人物运动视频以生成人物动作模板（可参考文档：[AnimateAnyone 动作模板生成](/zh/model-studio/animate-anyone-template-api)）。
    
    c. 调用“舞动人像AnimateAnyone”模型输入通过检测的人物图像和人物动作模板ID以生成视频（可参考文档：[AnimateAnyone 视频生成](/zh/model-studio/animateanyone-video-generation-api)）。
...
-   模型调用时，需在入参的“model”字段中填入**部署成功的模型名称**，并参照以下调用顺序：
    ...
    b. 调用“舞动人像AnimateAnyone”模型输入通过检测的人物图像和预设动作模板文件以生成视频（可参考文档：[AnimateAnyone 视频生成](/zh/model-studio/animateanyone-video-generation-api)）。
