...
写入终态；取消请求网络失败时也会恢复轮询，避免节点进入不可恢复的假停止状态。
...
- `GET /api/canvases/:id/executions` 通过 `task-repository.listTasks` 按用户和任务输入中的 `documentId`
  ...
  记录时重新读取该执行快照并恢复节点的稳定资产结果，不修改画布编辑快照。页面刷新时会先查询当前
  `documentRevision` 的 queued/running 任务并自动恢复 SSE/轮询；旧 revision 的运行任务不自动写回当前画布。
