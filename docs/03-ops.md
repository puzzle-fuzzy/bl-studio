...
观测栈启动并通过冒烟后，可在目标服务器上采集窗口基线：

```bash
bun run prod:observability:baseline -- 24
```

命令默认采集最近 24 小时，也可传入 1–168 小时整数。它从远端宿主机的 Loki 本地端口读取 Canvas 稳定事件，输出 JSON 汇总（执行量、终态结果、节点错误码、缓存命中和耗时 p50/p95/p99）；空窗口会明确标记无数据，不会自动写入告警规则。输出可重定向保存为发布记录，但当前仍需真实目标服务器完成 24–72 小时采集后再人工确定阈值。
