...
- [x] 完成资产详情的所属项目管理：支持查看项目归属、多选增删已有项目、按差异提交并刷新详情，项目标签可反向进入项目详情。
- [x] 完成资产工作台无障碍收口：搜索与筛选控件补齐关联标签和 `title`，图标按钮补齐 `aria-label`/`title`，缩略图补齐语义化 `alt`，加载/错误/选择状态使用可读的状态语义，并保留键盘焦点样式。
- [x] 重构应用侧栏交互：切换按钮移至内容区顶部独立工具条并保持正方形，侧栏改用登录页完整字标与收缩态 `BS` 标记，展开态 Logo 水平居中；普通导航项与通知/积分/账户入口统一收缩态尺寸、居中规则，并保留 tooltip 与可访问名称。
- [x] 统一素材库工具栏控件高度：项目选择、新建/生成/上传和视图切换共用 42px 基线，保留键盘焦点和响应式换行。
- [ ] 为资产工作台补页面级 E2E 与真实浏览器验收；当前单元/API/仓储和生产构建门禁已覆盖。
- [ ] 后续再规划剧本上传/生成与人物、场景、道具提示词分析，不把剧本逻辑塞进资产 repository 或 Provider executor。

当前执行顺序：开发环境 migration 基线已收口；项目上下文、最新版本状态筛选、项目详情批量整理、资产详情所属项目管理、侧栏响应式收口和素材库工具栏高度统一已完成。下一步进入有效登录态下的页面级浏览器验收与布局调整；本阶段继续不写页面级 UI 测试、不调用真实 Provider/API，也不修改导演、剧本和剪辑流程。
