...
当前执行顺序：开发环境 migration 基线已收口；项目上下文、最新版本状态筛选、项目详情批量整理、资产详情所属项目管理和侧栏响应式收口已完成。下一步进入有效登录态下的页面级浏览器验收与布局调整；本阶段继续不写页面级 UI 测试、不调用真实 Provider/API，也不修改导演、剧本和剪辑流程。
