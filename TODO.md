...
- [x] 调整项目与素材库的信息架构：新增独立 `/projects` 项目列表和 `/projects/:projectId` 项目详情入口，素材库移除“项目”页签；旧 `/assets/projects/:projectId` 保留兼容访问。
- [x] 收敛资产工作台模块：移除“风格”和“待确认”页签及风格创建/引用入口；底层 `style` 类型和候选版本状态保留用于已有数据兼容与资产详情审阅。
...
- [x] 修正收缩侧栏通知徽标裁切：仅在图标收缩态允许触发器溢出显示，未读数支持多位数字宽度并保持右上角定位。
- [x] 统一素材库工具栏控件高度：项目选择、新建/生成/上传和视图切换共用 42px 基线，保留键盘焦点和响应式换行。
- [x] 统一工作台、素材库和生成任务页面内容区宽度：全部使用素材库的 `w-full max-w-[1660px]` 基线，工作台移除额外横向收窄。
- [x] 统一工作台、素材库和生成任务页面结构内间距：页面层级使用 20px，标题区使用 16px 内间距和 20px 底部留白，工作台与生成任务复用素材库的标题区节奏。
- [x] 统一应用主内容滚动：主页面滚动接入现有 OverlayScrollbars 自定义主题，移除 AppShell 主内容层的原生 `overflow-auto`，保留虚拟列表和弹窗内部的专用滚动。
- [x] 收口页面内滚动：作品库视图、导演台清单、生成结果和 Radix `ScrollArea` 统一接入 OverlayScrollbars；写作、虚拟列表、表格、弹窗等第三方滚动节点统一套用同一套细 handle 视觉。
- [x] 统一应用内容区氛围：将右上角点阵水印提升为全局 `SidebarInset` 装饰层，移除工作台、项目列表和项目详情中的重复渐变/点阵实现。
- [ ] 为资产工作台补页面级 E2E 与真实浏览器验收；当前单元/API/仓储和生产构建门禁已覆盖。
- [ ] 后续再规划剧本上传/生成与人物、场景、道具提示词分析，不把剧本逻辑塞进资产 repository 或 Provider executor。

当前执行顺序：开发环境 migration 基线已收口；项目上下文、最新版本状态筛选、项目详情批量整理、独立项目入口、资产模块收敛、资产详情所属项目管理、侧栏响应式收口和素材库工具栏高度统一已完成。下一步进入有效登录态下的页面级浏览器验收与布局调整；本阶段继续不写页面级 UI 测试、不调用真实 Provider/API，也不修改导演、剧本和剪辑流程。
...
资产工作台本轮已完成：草稿版本允许添加/移除参考图并送入确认队列；候选版本可确认或驳回；已确认版本与其参考图保持不可变，只能建立新版本调整。生成页可选择主体、场景和道具的已确认资产，将版本 ID、参考图 ID 与用户提示词编译为 `creativeContext`，同时保留原有媒体参数兼容路径。Docker workspace/runtime 依赖缓存清单已补齐创意资产包。
