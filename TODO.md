...
当前执行顺序：开发环境 migration 基线已收口；本轮已完成项目上下文和最新版本状态筛选，下一步进入“项目详情与批量整理”评估：明确项目详情路由、添加/移除已有资产、批量选择和操作栏的协议边界。页面布局继续保持可调整，本阶段不做页面级 UI 测试、不调用真实 Provider/API，也不修改导演、剧本和剪辑流程。
