...
deploy_rsync "$DEPLOY_SSH_KEY" "$DEPLOY_SSH_KNOWN_HOSTS" "$ENV_PROD" "$DEPLOY_HOST:$DEPLOY_REMOTE_DIR/"
ssh_cmd "chmod 600 $DEPLOY_REMOTE_DIR/.env.production"

# 4) 重启 api/worker 应用新 key
echo "==> 重启 api/worker"
ssh_cmd "docker compose --env-file $DEPLOY_REMOTE_DIR/.env.prod-infra -f $REMOTE_DEPLOY/docker/compose.prod.yaml up -d --force-recreate --no-deps api worker"

# 5) 校验（md5 一致 + 注入长度，不打印值）
LOCAL_MD5="$(md5 -q "$ENV_PROD")"
REMOTE_MD5="$(ssh_cmd "md5sum $DEPLOY_REMOTE_DIR/.env.production | awk '{print \$1}'")"
