...
#   - .env.production 与 .env.prod-infra 已填真实值（gitignored）。
...
ENV_BACKUP="$REPO_ROOT/.env.prod-backup"
...
pnpm exec tsx scripts/backup/prepare-backup-env.ts \
...
# 环境：test DB 连接串来自 .env.test（测试库 :55432）。
...
if [[ "${DEPLOY_SKIP_VERIFY:-0}" != "1" ]]; then
  if [[ -f "$REPO_ROOT/.env.test" ]]; then
    ...
    pnpm exec dotenv -e .env.test -- pnpm run verify \
      || fail "verify 未通过；修好后重新部署（紧急时可 DEPLOY_SKIP_VERIFY=1 绕过，需自行评估）"
  else
    fail "缺少 .env.test：verify 门禁需要 test DB 连接串（参考 .env.test.example）"
  fi
else
...
# ── 预检 3：生产预检（不联网、不打印值）──────────────────────────
pnpm exec dotenv -e .env.production -- tsx scripts/verify/check-production-env.ts \
  || fail "check:production-env 未通过"
pnpm exec dotenv -e .env.prod-infra -- tsx scripts/verify/check-production-env.ts infra \
...
deploy_rsync "$DEPLOY_SSH_KEY" "$DEPLOY_SSH_KNOWN_HOSTS" "$ENV_APP" "$ENV_INFRA" "$ENV_BACKUP" "$DEPLOY_HOST:$DEPLOY_REMOTE_DIR/"

# 服务器侧收紧 env 文件权限（含真实凭据）。
ssh_cmd "chmod 600 $DEPLOY_REMOTE_DIR/.env.production $DEPLOY_REMOTE_DIR/.env.prod-infra $DEPLOY_REMOTE_DIR/.env.prod-backup"
