...
#   - deploy/env/.env.prod 已填真实值（gitignored）。
#   - verify 门禁：脚本会在干净工作区上强制跑 `bun run verify`（test DB 环境），
...
# ── 预检 1：本地环境文件存在 ──────────────────────────────────────
[[ -f "$ENV_PROD" ]] || fail "缺少 ${ENV_PROD}（复制 deploy/env/.env.example 后填写）"

# ── 预检 2：干净工作区 + commit SHA ──────────────────────────────
git diff --quiet && git diff --cached --quiet || fail "工作区有未提交改动；生产镜像只能从干净工作区构建"
SHA="$(git rev-parse HEAD)"

# 把当前 SHA 幂等写入统一生产 env 的 BAILIAN_STUDIO_RELEASE_TAG 行。
inject_release_tag() {
  local file="$1"
...
# ── 预检 3：生产预检（不联网、不打印值）──────────────────────────
bun x dotenv -e "$ENV_PROD" -- bun x tsx scripts/verify/check-production-env.ts \
  || fail "check:production-env 未通过"
bun x dotenv -e "$ENV_PROD" -- bun x tsx scripts/verify/check-production-env.ts infra \
...
SITE_DOMAIN="$(env_value SITE_DOMAIN "$ENV_PROD")"
LEGAL_ENTITY="$(env_value VITE_LEGAL_ENTITY "$ENV_PROD")"
LEGAL_CONTACT_EMAIL="$(env_value VITE_LEGAL_CONTACT_EMAIL "$ENV_PROD")"
LEGAL_EFFECTIVE_DATE="$(env_value VITE_LEGAL_EFFECTIVE_DATE "$ENV_PROD")"
# 目标服务器 CPU 架构。本机（Apple Silicon）与服务器（x86_64）架构不同时，
# 必须显式指定服务器架构跨平台构建，否则产出的 arm64 镜像无法在 x64 上运行。
# 若本机与服务器同为 x86_64，可在 .env.prod 置空走默认 linux/amd64。
DEPLOY_PLATFORM="$(env_value DEPLOY_PLATFORM "$ENV_PROD")"
...
# 上一次提交只动了前端 app（docs 不计入），全量部署会白传 runtime 镜像。
changed_files="$(git diff --name-only "$SHA~1" "$SHA" 2>/dev/null || true)"
non_frontend="$(printf '%s\n' "$changed_files" | grep -vE '^(apps/(studio|writer|canvas|admin)|docs)/' | grep -v '^$' || true)"
if [[ -n "$changed_files" && -z "$non_frontend" ]] && printf '%s\n' "$changed_files" | grep -qE '^apps/(studio|writer|canvas|admin)/'; then
  echo "==> 提示：本次提交只改了前端 app（studio/writer/canvas/admin）。"
  echo "==> 前端热修建议用 bun run deploy:prod:web（传输 ~20MB，不重传 runtime 镜像）。"
...
# deploy/docker/compose.prod.yaml 的 x-ffmpeg-mounts 保持一致。
FFMPEG_HOST_DIR="$(env_value FFMPEG_HOST_DIR "$ENV_PROD")"
...
if ! ssh_cmd "test -x ${FFMPEG_HOST_DIR}/ffmpeg && test -x ${FFMPEG_HOST_DIR}/ffprobe"; then
  fail "服务器缺少静态 ffmpeg/ffprobe（${FFMPEG_HOST_DIR}）。先运行：scripts/dev/fetch-static-ffmpeg.sh"
fi

# ── 本机构镜像（SHA tag，不可变）─────────────────────────────────
echo "==> 构建 runtime / web / backup 镜像（平台 ${DEPLOY_PLATFORM}）"
docker build --platform "$DEPLOY_PLATFORM" -f deploy/docker/Dockerfile --target runtime \
  --build-arg BAILIAN_STUDIO_RELEASE_TAG="$SHA" \
  -t "bailian-studio-runtime:$SHA" .
docker build --platform "$DEPLOY_PLATFORM" -f deploy/docker/Dockerfile --target web \
  --build-arg BAILIAN_STUDIO_RELEASE_TAG="$SHA" \
...
docker build --platform "$DEPLOY_PLATFORM" -f deploy/docker/Dockerfile.backup \
  --build-arg BAILIAN_STUDIO_RELEASE_TAG="$SHA" \
...
ssh_cmd "mkdir -p $REMOTE_DEPLOY/docker $REMOTE_DEPLOY/observability/loki $REMOTE_DEPLOY/observability/alloy $REMOTE_DEPLOY/observability/grafana $REMOTE_DEPLOY/nginx $REMOTE_DEPLOY/scripts"

deploy_rsync "$DEPLOY_SSH_KEY" "$DEPLOY_SSH_KNOWN_HOSTS" "images-$SHA.tar" "$DEPLOY_HOST:$DEPLOY_REMOTE_DIR/"
deploy_rsync "$DEPLOY_SSH_KEY" "$DEPLOY_SSH_KNOWN_HOSTS" deploy/docker/compose.prod.yaml "$DEPLOY_HOST:$REMOTE_DEPLOY/docker/"
# 三个目录分别同步到自己的目标目录；多源 rsync 指向同一目标会在服务器
# 已存在错误路径时把应为单文件的 config.alloy/loki.yaml 留成目录，破坏单文件挂载。
deploy_rsync "$DEPLOY_SSH_KEY" "$DEPLOY_SSH_KNOWN_HOSTS" deploy/observability/loki/ "$DEPLOY_HOST:$REMOTE_DEPLOY/observability/loki/"
deploy_rsync "$DEPLOY_SSH_KEY" "$DEPLOY_SSH_KNOWN_HOSTS" deploy/observability/alloy/ "$DEPLOY_HOST:$REMOTE_DEPLOY/observability/alloy/"
deploy_rsync "$DEPLOY_SSH_KEY" "$DEPLOY_SSH_KNOWN_HOSTS" deploy/observability/grafana/ "$DEPLOY_HOST:$REMOTE_DEPLOY/observability/grafana/"
# 宿主机 nginx 边缘：容器内 nginx 配置（烘焙进镜像）+ 两个站点模板 + 边缘接入脚本。
deploy_rsync "$DEPLOY_SSH_KEY" "$DEPLOY_SSH_KNOWN_HOSTS" deploy/nginx/ "$DEPLOY_HOST:$REMOTE_DEPLOY/nginx/"
deploy_rsync "$DEPLOY_SSH_KEY" "$DEPLOY_SSH_KNOWN_HOSTS" scripts/deploy/setup-host-edge.sh scripts/dev/fetch-static-ffmpeg.sh scripts/deploy/production-monitor.sh scripts/deploy/restore-rehearsal.sh "$DEPLOY_HOST:$REMOTE_DEPLOY/scripts/"
...
ssh_cmd "bash $REMOTE_DEPLOY/scripts/setup-host-edge.sh $REMOTE_DEPLOY"
...
bash "$REPO_ROOT/scripts/verify/verify-api-release.sh" "$SHA" "$SITE_DOMAIN" "$SERVER_HOST"
rm -f "images-$SHA.tar"
...
echo "==> 日志查看：https://$(env_value LOGS_DOMAIN "$ENV_PROD")"
